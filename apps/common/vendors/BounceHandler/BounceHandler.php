...
class BounceHandler
{
    ...
    /**
     * This is not used anymore starting with 2.0.33
     * 
     * @var array
     */
     ...
     * @var array
     */
    public $imapOpenParams = [];
	
	/** @var callable */
	public $isValidResultCallback;
    ...
    public const FEEDBACK_LOOP_REPORT = 'feedback-loop-report';

    public const DIAGNOSTIC_CODE_RULES = "DIAGNOSTIC_CODE_RULES";

    public const DSN_MESSAGE_RULES = "DSN_MESSAGE_RULES";

    public const BODY_RULES = "BODY_RULES";

    public const COMMON_RULES = "COMMON_RULES";
	 ...
	 */
    public function __construct($connectionString = '', $username = '', $password = '', array $options = [])
    {
    ...
    public function getResults()
    {
        ...
        foreach ($searchResults as $messageId) {
            ...
            $result = ['email'                     => null, 'bounceType'                => self::BOUNCE_INTERNAL, 'action'                    => null, 'statusCode'                => null, 'diagnosticCode'            => 'BOUNCED BACK', 'headers'                   => null, 'body'                      => null, 'originalEmail'             => null, 'originalEmailHeadersArray' => []];
	        ...
	        $valid = true;
			if ($this->isValidResultCallback !== null && is_callable($this->isValidResultCallback)) {
				/** @var callable $callback */
				$callback = $this->isValidResultCallback;
				$valid = (bool)call_user_func($callback, $result);
			}
    ...
    public function getHeadersArray($rawHeader)
    {
        ...
        if (!empty($headerLines[0])) {
            foreach ($headerLines[0] as $line) {
                ...
                $headers[$name] = html_decode((string)trim((string)$value));
            }
        }
    ...
    protected function processDsn($messageId)
    {
        ...
        if (preg_match ("/Action: (.+)/i", $dsnReport, $matches)) {
            $action = strtolower(trim((string)$matches[1]));
        }
    ...
    protected function extractBody($messageId)
    {
        ...
        if (!empty($structure)) {
            if (in_array($structure->type, [0, 1])) {
    ...
    public function getSearchResults()
    {
        ...
        if (empty($this->searchString)) {
            $this->searchString = sprintf('UNDELETED SINCE "%s"', date('d-M-Y', (int)strtotime(sprintf('-%d days', (int)$this->processDaysBack))));
        }
    ...
    protected function getRules()
    {
        ...
        if (empty(self::$_rules)) {
            self::$_rules = require(__DIR__ . '/rules.php');
        }

        if (is_file($customRulesFile = __DIR__ . '/rules-custom.php')) {
        ...
        self::$_rules[self::COMMON_RULES]          = [];
        
        // since 1.3.6.3
        if (is_file($customRulesFile = __DIR__ . '/rules-custom-override.php')) {
    ...
    public static function stripSpecialChars($text)
    {
	    ...
	    $text = (string)str_replace("doesn\'t", "doesn't", $text);
