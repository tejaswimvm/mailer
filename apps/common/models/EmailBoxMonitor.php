...
class EmailBoxMonitor extends BounceServer
{
    ...
    public const ACTION_UNCONFIRM             = 'unconfirm';
    public const ACTION_DELETE                = 'delete';
    public const ACTION_MOVE_TO_LIST          = 'move to list';
    public const ACTION_COPY_TO_LIST          = 'copy to list';
    public const ACTION_STOP_CAMPAIGN_GROUP   = 'stop campaign group';

    /**
     * Identify list
     */
    public const IDENTIFY_SUBSCRIBERS_BY_EMAIL     = 'by email address';
    public const IDENTIFY_SUBSCRIBERS_BY_UID       = 'by subscriber uid';
    public const IDENTIFY_SUBSCRIBERS_UID_OR_EMAIL = 'by subscriber uid or email address';
    ...
    {
        $mutexKey = sha1('imappop3box' . SerializeHelper::serialize($this->getAttributes(['hostname', 'username', 'password'])) . date('Ymd'));
        ...
        try {
            ...
            foreach ($results as $result) {
                ...
                // since 2.3.8 - look in the subject as well
                $messageLines = [
                    (string) $parsedMessage->getHeaderValue('subject'),
                ];
                ...
                // since 2.3.8
                $contents = (string)$plainText->getContents();
                if ($plainText->getHeaderValue('content-transfer-encoding') === 'base64') {
                    $contents = (string)base64_decode($contents);
                } elseif ($plainText->getHeaderValue('content-transfer-encoding') === 'quoted-printable') {
                    $contents = (string)quoted_printable_decode($contents);
                }

                $lines = array_filter(array_unique(array_map('trim', (array)explode(PHP_EOL, $contents))));
                ...
                $messageContent = implode(PHP_EOL, $messageLines);
                //
                ...
                if ($identifySubscribersBy == self::IDENTIFY_SUBSCRIBERS_BY_UID || $identifySubscribersBy == self::IDENTIFY_SUBSCRIBERS_UID_OR_EMAIL) {
                    ...
                    // since 2.3.8: Subject
                    if (empty($campaign) && empty($subscriber) && preg_match_all('/(Subject): ([^\r\n]+)/im', $message, $matches)) {
                        $subjects = array_reverse(!empty($matches[2]) && is_array($matches[2]) ? $matches[2] : []);
                        foreach ($subjects as $subject) {
                            if (!preg_match('/campaign\-uid:([a-z0-9]+)((\s+)?\/(\s+)?)?subscriber\-uid:([a-z0-9]+)/i', $subject, $matches)) {
                                continue;
                            }
                            ...
                            /** @var ListSubscriber|null $subscriber */
                            $subscriber = ListSubscriber::model()->findByAttributes(['subscriber_uid' => $matches[5]]);
                    ...
                    if (empty($campaign) && empty($subscriber) && preg_match('/\/([a-z0-9]{13})\/track\-opening\/([a-z0-9]{13})/ix', $message, $matches)) {
                        /** @var Campaign|null $campaign */
                        $campaign = Campaign::model()->findByAttributes(['campaign_uid' => $matches[1]]);
                    ...
                    if (empty($campaign) && empty($subscriber) && preg_match_all('/(In\-Reply\-To): ([^\r\n]+)/im', $message, $matches)) {
                        ...
                        foreach ($messageIds as $messageId) {
                            ...
                            $log = CampaignDeliveryLogHelper::findByEmailMessageId((string)$messageId);
                            if (!empty($log)) {
                ...
                foreach ($subscribers as $subscriber) {
                    ...
                    // since 2.2.12
                    try {
                        hooks()->doAction('emailboxmonitor_process_remote_contents_before_subscriber_action', $subscriber, $campaign, $condition);
                    } catch (Exception $e) {
                    ...
                    // since 2.2.12
                    try {
                        hooks()->doAction('emailboxmonitor_process_remote_contents_after_subscriber_action', $subscriber, $campaign, $condition);
                    } catch (Exception $e) {
                        Yii::log($e->getMessage(), CLogger::LEVEL_ERROR);
                    }
                }
