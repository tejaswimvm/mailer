...
class BounceServer extends ActiveRecord
{
     ...
     * Flag
     */
    public const STATUS_CRON_RUNNING = 'cron-running';
    ...
    public function processRemoteContents(array $params = []): bool
    {
        ...
        $mailBoxes = array_filter(array_unique($mailBoxes));
    ...
    public function getDeliveryLogFromBounceHandlerResult(array $result): ?CampaignDeliveryLog
    {
        ...
        foreach ($headers as $key => $value) {
            ...
            $log = CampaignDeliveryLogHelper::findByEmailMessageId((string)$value);
            if (!empty($log)) {
                return $log;
            }
        }
    ...
    protected function _processRemoteContents(array $params = []): bool
    {
        ...
        $mutexKey = sha1('imappop3box' . SerializeHelper::serialize($this->getAttributes(['hostname', 'username', 'password'])));
