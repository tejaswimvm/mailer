 ...
 * @property ListSubscriberMetaData $customMetaData
 *
 * Getters:
 * @property bool $isUnconfirmed
 * @property bool $isConfirmed
 */
class ListSubscriber extends ActiveRecord implements ListIdentifierInterface, CampaignIdentifierInterface, DynamicListFieldValueInterface, DynamicCampaignDeliveryLogInterface
{
     ...
     * Bulk actions
     */
    public const BULK_SUBSCRIBE = 'subscribe';
    public const BULK_UNSUBSCRIBE = 'unsubscribe';
    public const BULK_DISABLE = 'disable';
    public const BULK_DELETE = 'delete';
    public const BULK_BLACKLIST = 'blacklist';
    public const BULK_BLACKLIST_IP = 'blacklist-ip';
    public const BULK_UNCONFIRM = 'unconfirm';
    public const BULK_RESEND_CONFIRMATION_EMAIL = 'resend-confirmation-email';

    /**
     * Campaign filters
     */
    public const CAMPAIGN_FILTER_ACTION_DID_OPEN = 1;
    public const CAMPAIGN_FILTER_ACTION_DID_CLICK = 2;
    public const CAMPAIGN_FILTER_ACTION_DID_NOT_OPEN = 3;
    public const CAMPAIGN_FILTER_ACTION_DID_NOT_CLICK = 4;

    /**
     * General filters
     */
    public const FILTER_TIME_UNIT_DAY = 1;
    public const FILTER_TIME_UNIT_WEEK = 2;
    public const FILTER_TIME_UNIT_MONTH = 3;
    public const FILTER_TIME_UNIT_YEAR = 4;

    /**
     * Meta data keys
     */
    public const META_DATA_CONFIRM_SUBSCRIBE_KEY_NAME = 'self.subscribe.confirm_subscribe_key';
    public const META_DATA_PROFILE_LAST_UPDATED_KEY_NAME = 'self.profile.last_updated';
    public const META_DATA_CONFIRM_UPDATE_PROFILE_KEY_NAME = 'self.profile.confirm_update_key';
    ...
    public function relations()
    {
        $relations = [
            ...
            'customMetaData'        => [self::HAS_MANY, ListSubscriberMetaData::class, 'subscriber_id'],
        ];
    ...
    public function copyToList(int $listId, bool $doTransaction = true, bool $notify = false): ?self
    {
        ...
        try {
            ...
            /** @var array<ListField[]> $fieldModels */
            ...
            foreach ($fieldModels as $models) {
                ...
                $sourceValues = $this->getListFieldValueModel()->findAllByAttributes([
                ...
                $isOptionsAware = $target->getIsOptionsAware() && !empty($target->options);
                $targetOptions  = $isOptionsAware ? collect($target->options) : null;

                foreach ($sourceValues as $sourceValue) {
                    $sourceValueValue = $sourceValue->value;

                    $isValidValue = true;
                    if ($isOptionsAware && !empty($targetOptions)) {
                        // find the actual option, because that's the value we have to insert
                        /** @var ListFieldOption $targetValueOption */
                        $targetValueOption = $targetOptions->filter(function (ListFieldOption $targetValueOption) use ($sourceValue) {
                            return strtolower((string)$targetValueOption->value) === strtolower((string)$sourceValue->value);
                        })->first();

                        $isValidValue = !empty($targetValueOption);

                        // if we found a valid option, the value to be saved becomes the option's value
                        if ($isValidValue) {
                            $sourceValueValue = (string) $targetValueOption->value;
                        }
                    }

                    // allow only valid options from the target fields when copying a subscriber
                    if (!$isValidValue) {
                        continue;
                    }

                    /** @var ListFieldValue $targetValue */
                    $targetValue = $subscriber->createListFieldValueInstance();

                    $targetValue->field_id      = (int)$target->field_id;
                    $targetValue->subscriber_id = (int)$subscriber->subscriber_id;
                    $targetValue->value         = $sourceValueValue;
    ...
    public function loadAllCustomFieldsWithValues(): array
    {
        ...
        foreach (ListField::getAllByListId((int)$this->list_id) as $field) {
            $values = db()->createCommand()
                ->select('value')
                ->from($this->getListFieldValueModel()->tableName())
            ...
            $fields['[' . $field['tag'] . ']'] = implode((string)$field['multi_values_separator'], $value);
        }
    ...
    public function updateGeoLocationFields()
    {
        ...
        foreach ($fields[(int)$this->list_id] as $field) {
            ...
            if (empty($value)) {
                $command->insert($this->getListFieldValueModel()->tableName(), $data);
            } else {
                unset($data['date_added']);
                if (empty($value['value'])) {
                    $command->update($this->getListFieldValueModel()->tableName(), $data, 'value_id = :vid', [':vid' => $value['value_id']]);
                }
            }
        }
    }
    ...
    public function handleFieldsDefaultValues(): void
    {
        ...
        foreach ($fields[(int)$this->list_id] as $field) {
            ...
            $defaultValue = ListField::parseDefaultValueTags((string)$field->default_value, $this);

            if ($field->getIsMultiValueField() && $field->importSubscriberMultiValues($this, $defaultValue)) {
                continue;
            }
    ...
    public function handleApprove(bool $forcefully = false): self
    {
        ...
        $searchReplace = [
            ...
            '[UPDATE_PROFILE_URL]'  => $this->getUpdateProfileUrl(),
        ...
        $sent = false;
        for ($i = 0; $i < 3; ++$i) {
            if ($sent = $server->setDeliveryFor(DeliveryServer::DELIVERY_FOR_LIST)->setDeliveryObject($this->list)->send($params)) {
                break;
            }
            if (!($server = DeliveryServer::pickServer((int)$server->server_id, $this->list, ['useFor' => [DeliveryServer::USE_FOR_LIST_EMAILS]]))) {
                break;
            }
        }
    ...
    public function handleWelcome(bool $forcefully = false): self
    {
        ...
        for ($i = 0; $i < 3; ++$i) {
            ...
            /** @var DeliveryServer|null $server */
            $server = DeliveryServer::pickServer((int)$server->server_id, $this->list, ['useFor' => [DeliveryServer::USE_FOR_LIST_EMAILS]]);
    ...
    public function sendCreatedNotifications(): self
    {
        ...
        foreach ($listFields as $field) {
            $fieldValues = $this->getListFieldValueModel()->findAll([
                'select'    => 'value',
        ...
        foreach ($recipients as $recipient) {
            ...
            $server->setDeliveryFor(DeliveryServer::DELIVERY_FOR_LIST)->setDeliveryObject($this->list)->send($params);
        }
    ...
    {
        if ($this->getIsUnsubscribed()) {
            return true;
        }

        $mutexKey = sha1(sprintf('subscriber:unsubscribe:%s', $this->subscriber_id));
     ...
     * @return bool
     */
    public function setCustomMetaData(string $key, $value): bool
    {
        $model = $this->getCustomMetaData($key);
        if (empty($model)) {
            $model = new ListSubscriberMetaData();
        }
     ...
     * @param string $key
     *
     * @return ListSubscriberMetaData|null
     */
    public function getCustomMetaData(string $key): ?ListSubscriberMetaData
    {
        /** @var ListSubscriberMetaData|null $model */
        $model = ListSubscriberMetaData::model()->findByAttributes([
            'subscriber_id' => (int)$this->subscriber_id,
            'key' => $key,
        ]);
     ...
     * @return string
     */
    public function getConfirmSubscribeKey(): string
    {
        if ($this->_confirmSubscribeKey === null) {
            $confirmationKeyModel = $this->getCustomMetaData(self::META_DATA_CONFIRM_SUBSCRIBE_KEY_NAME);
            $confirmationKey = $confirmationKeyModel ? $confirmationKeyModel->value : '';

            if (!$confirmationKeyModel) {
                $confirmationKey = StringHelper::randomSha1();
                $this->setCustomMetaData(self::META_DATA_CONFIRM_SUBSCRIBE_KEY_NAME, $confirmationKey);
            }

            $this->_confirmSubscribeKey = (string)$confirmationKey;
        }

        return (string)$this->_confirmSubscribeKey;
    }
     ...
     * @return bool
     */
    public function matchesConfirmSubscribeKey(string $confirmationKey): bool
    {
        return hash_equals($confirmationKey, $this->getConfirmSubscribeKey());
    }

    /**
     * @return string
     */
    public function getPendingSubscribeUrl(): string
    {
        // since 2.3.8
        // this way, if using custom domains, the redirect will go to the custom domain
        // instead of the base domain where the app is installed.
        if (apps()->isAppName('frontend')) {
            return createAbsoluteUrl('lists/subscribe_pending', [
                'list_uid' => $this->list->list_uid,
                'subscriber_uid' => $this->subscriber_uid,
            ]);
        }
        ...
        return $optionUrl->getFrontendUrl('lists/' . $this->list->list_uid . '/pending-subscribe/' . $this->subscriber_uid);
    }
     ...
     * @return string
     */
    public function getConfirmSubscribeUrl(string $append = ''): string
    {
        // since 2.3.8
        // this way, if using custom domains, the redirect will go to the custom domain
        // instead of the base domain where the app is installed.
        if (apps()->isAppName('frontend')) {
            return createAbsoluteUrl('lists/subscribe_confirm', [
                'list_uid' => $this->list->list_uid,
                'subscriber_uid' => $this->subscriber_uid,
                'confirmation_key' => $this->getConfirmSubscribeKey(),
            ]) . $append;
        }
        ...
        return $optionUrl->getFrontendUrl(sprintf(
            'lists/%s/confirm-subscribe/%s/%s%s',
            $this->list->list_uid,
            $this->subscriber_uid,
            $this->getConfirmSubscribeKey(),
            $append
        ));
    }
    ...
    public function getUnsubscribeUrl(): string
    {
        ...
        // instead of the base domain where the app is installed.
        if (apps()->isAppName('frontend')) {
            return createAbsoluteUrl('lists/unsubscribe', [
                'list_uid' => $this->list->list_uid,
                'subscriber_uid' => $this->subscriber_uid,
            ]);
        }
        ...
        return $optionUrl->getFrontendUrl('lists/' . $this->list->list_uid . '/unsubscribe/' . $this->subscriber_uid);
    }

    /**
     * @return null|string
     */
    public function getUpdateProfileKey(): ?string
    {
        $confirmationKeyModel = $this->getCustomMetaData(self::META_DATA_CONFIRM_UPDATE_PROFILE_KEY_NAME);
        $confirmationKey = $confirmationKeyModel ? (string)$confirmationKeyModel->value : '';
        if (stripos($confirmationKey, '_') !== false) {
            [$confirmationKey, $generatedAt] = explode('_', $confirmationKey);
        } else {
            $confirmationKey = $generatedAt = null;
        }

        if ($confirmationKey && (int)$generatedAt + 3600 < time()) {
            $this->deleteUpdateProfileKey();
            return null;
        }
     ...
     * @return string
     */
    public function generateUpdateProfileKey(): string
    {
        $confirmationKey = strtoupper((string)StringHelper::random(8));
        $generatedAt = time();
        $this->setCustomMetaData(self::META_DATA_CONFIRM_UPDATE_PROFILE_KEY_NAME, sprintf('%s_%d', $confirmationKey, $generatedAt));
     ...
     * @return bool
     */
    public function deleteUpdateProfileKey(): bool
    {
        return $this->setCustomMetaData(self::META_DATA_CONFIRM_UPDATE_PROFILE_KEY_NAME, '');
    }
    ...
    public function getUpdateProfileUrl(): string
    {
        ...
        // instead of the base domain where the app is installed.
        if (apps()->isAppName('frontend')) {
            return createAbsoluteUrl('lists/update_profile', [
                'list_uid' => $this->list->list_uid,
                'subscriber_uid' => $this->subscriber_uid,
            ]);
        }
        ...
        return $optionUrl->getFrontendUrl('lists/' . $this->list->list_uid . '/update-profile/' . $this->subscriber_uid);
    }
