...
class CustomerEmailTemplate extends ActiveRecord
{
    ...
    public function rules()
    {
        ...
        $rules =  [
            ...
            // since 2.4.2
            ['content', '_validateContent', 'on' => 'insert, update'],
    ...
    public function copy(): ?self
    {
        ...
        $template = $this->createNewInstanceFromLoadedAttributes();
        $template->template_uid = $templateUid;
        $template->content      = (string)str_replace($this->template_uid, $templateUid, $this->content);
        $template->content_hash = '';
        $template->screenshot   = (string)preg_replace('#' . $this->template_uid . '#', $templateUid, (string)$this->screenshot, 1);
     ...
     * @return string
     */
    public function getScreenshotSrc(int $width = 600, int $height = 775): string
    {
        if (!empty($this->screenshot)) {
            ...
            try {
                if ($image = @ImageHelper::resize((string)$this->screenshot, $width, $height)) {
                    return $image;
                }
            } catch (Exception $e) {
            }
    ...
    {
        return (new CampaignTemplate())->getAvailableTags();
    }
     ...
     * @param array $params
     */
    public function _validateContent(string $attribute, array $params): void
    {
        if (!empty($this->$attribute) && CampaignHelper::isTemplateEngineEnabled()) {
            try {
                CampaignHelper::parseByTemplateEngineWithException($this->$attribute);
