...
class Page extends ActiveRecord
{
    ...
    public function generateSlug(): string
    {
        ...
        while (!empty($exists)) {
            ...
            $slug = URLify::filter((string)$slug . ' ' . $i);
            $criteria = new CDbCriteria();
            $criteria->addCondition('page_id != :id AND slug = :slug');
            $criteria->params = [':id' => $page_id, ':slug' => $slug];
            $exists = self::model()->find($criteria);
     ...
     * @return string
     */
    public function getTitleWithSlug(): string
    {
        if (empty($this->title)) {
            return t('pages', 'Please choose a title and slug');
        }
        if (empty($this->slug)) {
            return $this->title;
        }

        return sprintf('%s (%s)', $this->title, $this->slug);
    }
