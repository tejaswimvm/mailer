 ...
 * @property string|null $send_between_start
 * @property int|null $send_between_interval
 ...
 */
class Campaign extends ActiveRecord implements CampaignIdentifierInterface, DynamicCampaignDeliveryLogInterface
{
    use CampaignIdentifierTrait;
    use DynamicCampaignDeliveryLogTrait;

    /**
     * Statuses list
     */
    public const STATUS_DRAFT = 'draft';
    public const STATUS_PENDING_SENDING = 'pending-sending';
    ...
    public const STATUS_PENDING_APPROVE = 'pending-approve';
     ...
     * Bulk actions list
     */
    public const BULK_ACTION_PAUSE_UNPAUSE = 'pause-unpause';
    public const BULK_ACTION_MARK_SENT = 'mark-sent';
    public const BULK_EXPORT_BASIC_STATS = 'export-basic-stats';
    public const BULK_ACTION_SEND_TEST_EMAIL = 'send-test-email';
    public const BULK_ACTION_SHARE_CAMPAIGN_CODE = 'share-campaign-code';
    public const BULK_ACTION_COMPARE_CAMPAIGNS = 'compare-campaigns';
    public const BULK_ACTION_ARCHIVE_UNARCHIVE_CAMPAIGNS = 'archive-unarchive-campaigns';
     ...
     * @var null|bool
     */
    protected $_isAutoresponderDependency;
    ...
    public function rules()
    {
        $rules = [
            ...
            // since 2.2.0
            ['send_between_start', 'date', 'format' => 'hh:mm:ss', 'on' => 'step-confirm'],
            ['send_between_start', 'in', 'range' => array_keys($this->getSendBetweenHoursList()), 'on' => 'step-confirm'],
            ['send_between_start', '_validateSendBetweenStart', 'on' => 'step-confirm'],
            // since 2.2.3
            ['send_between_interval', 'numerical', 'integerOnly' => true, 'on' => 'step-confirm'],
            ['send_between_interval', 'in', 'range' => array_keys($this->getSendBetweenIntervalsList()), 'on' => 'step-confirm'],
            ['send_between_interval', '_validateSendBetweenInterval', 'on' => 'step-confirm'],

            // The following rule is used by search().
            ['campaign_uid, customer_id, group_id, send_group_id, list_id, name, type, status, search_recurring, search_ar_event, search_ar_time, search_template_name, is_archived', 'safe', 'on'=>'search'],
        ];
    ...
    public function attributeLabels()
    {
        $labels = [
            ...
            'send_between_start'    => $this->t('Daily sending window - Start hour'),
            'send_between_interval' => $this->t('Daily sending window - Interval'),
    ...
    public function search()
    {
        ...
        $expression = '
            ...
            CASE
              WHEN `t`.`type`=\'autoresponder\' AND `option`.`autoresponder_time_unit`=\'year\' THEN DATE_ADD(NOW(), INTERVAL @n YEAR)
              WHEN `t`.`type`=\'autoresponder\' AND `option`.`autoresponder_time_unit`=\'month\' THEN DATE_ADD(NOW(), INTERVAL @n MONTH)
              WHEN `t`.`type`=\'autoresponder\' AND `option`.`autoresponder_time_unit`=\'day\' THEN DATE_ADD(NOW(), INTERVAL @n DAY)
              WHEN `t`.`type`=\'autoresponder\' AND `option`.`autoresponder_time_unit`=\'hour\' THEN DATE_ADD(NOW(), INTERVAL @n HOUR)
              WHEN `t`.`type`=\'autoresponder\' AND `option`.`autoresponder_time_unit`=\'minute\' THEN DATE_ADD(NOW(), INTERVAL @n MINUTE)
              WHEN `t`.`type`=\'autoresponder\' AND `option`.`autoresponder_time_unit`=\'second\' THEN DATE_ADD(NOW(), INTERVAL @n SECOND)
              ELSE NOW()
        ...
        if (empty($this->is_archived)) {
            $criteria->compare('t.is_archived', '<>' . self::TEXT_YES);
        } else {
            $criteria->compare('t.is_archived', $this->is_archived);
        }
    ...
    public function attributeHelpTexts()
    {
        $texts = [
            ...
            'send_between_start'    => $this->t('Start sending the campaign at this hour'),
            'send_between_interval' => $this->t('Only for this amount of hours'),
    ...
    public function archive(): bool
    {
        ...
        $this->is_archived = self::TEXT_YES;
     ...
     * @return bool
     */
    public function archiveUnarchive(): bool
    {
        if ($this->getIsArchived()) {
    ...
    public function copy(bool $doTransaction = true): ?self
    {
        ...
        try {
            ...
            $campaign = $this->createNewInstanceFromLoadedAttributes();
            ...
            // campaign options
            /** @var CampaignOption $option */
            $option = !empty($this->option) ? $this->option->createNewInstanceFromLoadedAttributes() : new CampaignOption();
            ...
            foreach ($openActions as $action) {
                /** @var CampaignOpenActionSubscriber $action */
                $action = $action->createNewInstanceFromLoadedAttributes();
            ...
            foreach ($sentActions as $action) {
                /** @var CampaignSentActionSubscriber $action */
                $action = $action->createNewInstanceFromLoadedAttributes();
                $action->campaign_id  = (int)$campaign->campaign_id;
                $action->date_added   = MW_DATETIME_NOW;
                $action->last_updated = MW_DATETIME_NOW;
            ...
            foreach ($openWebhooks as $openWebhook) {
                /** @var CampaignTrackOpenWebhook $openWebhook */
                $openWebhook = $openWebhook->createNewInstanceFromLoadedAttributes();
            ...
            foreach ($urlWebhooks as $urlWebhook) {
                ...
                $urlWebhook = $urlWebhook->createNewInstanceFromLoadedAttributes();
            ...
            foreach ($extraTags as $extraTag) {
                ...
                $extraTag = $extraTag->createNewInstanceFromLoadedAttributes();
            ...
            foreach ($openListFieldActions as $action) {
                /** @var CampaignOpenActionListField $action */
                $action = $action->createNewInstanceFromLoadedAttributes();
                $action->campaign_id  = (int)$campaign->campaign_id;
                $action->date_added   = MW_DATETIME_NOW;
                $action->last_updated = MW_DATETIME_NOW;
            ...
            foreach ($sentListFieldActions as $action) {
                /** @var CampaignSentActionListField $action */
                $action = $action->createNewInstanceFromLoadedAttributes();
                $action->campaign_id  = (int)$campaign->campaign_id;
                $action->date_added   = MW_DATETIME_NOW;
                $action->last_updated = MW_DATETIME_NOW;
            ...
            if (!empty($this->template)) {
                ...
                $template = $this->template->createNewInstanceFromLoadedAttributes();
            } else {
            ...
            foreach ($randomContents as $randomContent) {
                ...
                $randomContent               = $randomContent->createNewInstanceFromLoadedAttributes();
            ...
            if (!empty($templateClickActions) || !empty($templateClickActionsListFields)) {
                $templateUrls = $template->getContentUrls(new CAttributeCollection(['campaign' => $campaign]));
                foreach ($templateClickActions as $clickAction) {
                    if (!in_array($clickAction->url, $templateUrls)) {
                        continue;
                    }
                    /** @var CampaignTemplateUrlActionSubscriber $clickAction */
                    $clickAction = $clickAction->createNewInstanceFromLoadedAttributes();
                ...
                foreach ($templateClickActionsListFields as $clickAction) {
                    ...
                    /** @var CampaignTemplateUrlActionListField $clickAction */
                    $clickAction = $clickAction->createNewInstanceFromLoadedAttributes();
                    $clickAction->campaign_id  = (int)$campaign->campaign_id;
                    $clickAction->template_id  = (int)$template->template_id;
                    $clickAction->date_added   = MW_DATETIME_NOW;
            ...
            if (!empty($this->attachments)) {
                ...
                if ($copiedAttachments) {
                    foreach ($this->attachments as $attachment) {
                        ...
                        $attachment = $attachment->createNewInstanceFromLoadedAttributes();
            ...
            foreach ($openUnopenFilters as $_openUnopenFilter) {
                /** @var CampaignFilterOpenUnopen $openUnopenFilter */
                $openUnopenFilter = $_openUnopenFilter->createNewInstanceFromLoadedAttributes();
                $openUnopenFilter->campaign_id = (int)$campaign->campaign_id;
                $openUnopenFilter->previous_campaign_id = (int)$_openUnopenFilter->previous_campaign_id;
    ...
    public function getSegmentsDropDownArray(): array
    {
        ...
        $criteria->addNotInCondition('t.status', [ListSegment::STATUS_PENDING_DELETE, ListSegment::STATUS_ARCHIVED]);
    ...
    public function getGroupsDropDownArray(): array
    {
        ...
        foreach ($models as $model) {
            $_options[(int)$model->group_id] = $model->name;
        }
    ...
    public function getRemovable(): bool
    {
        ...
        // We want that the customer permission not to be checked in backend and cli
        $checkForCustomer = !apps()->isAppName('backend') && !is_cli();

        if ($removable && $checkForCustomer && !empty($this->customer_id) && !empty($this->customer)) {
            $removable = $this->customer->getGroupOption('campaigns.can_delete_own_campaigns', 'yes') === 'yes';
        }

        // since 2.2.2
        if ($removable && $this->getIsAutoresponderDependency()) {
            $removable = false;
        }
     ...
     * @return bool
     */
    public function getIsAutoresponderDependency(): bool
    {
        if ($this->_isAutoresponderDependency !== null) {
            return $this->_isAutoresponderDependency;
        }

        $criteria = new CDbCriteria();
        $criteria->select = 't.campaign_id';
        $criteria->addCondition('(t.autoresponder_open_campaign_id = :cid OR t.autoresponder_sent_campaign_id = :cid)');
        $criteria->with = [];
        $criteria->with['campaign'] = [
            'select'    => 'campaign.campaign_id',
            'joinType'  => 'INNER JOIN',
            'together'  => true,
            'condition' => 'campaign.status != :status',
            'params'    => [':status' => Campaign::STATUS_PENDING_DELETE],
        ];
        $criteria->params[':cid'] = (int)$this->campaign_id;

        return $this->_isAutoresponderDependency = (CampaignOption::model()->find($criteria) !== null);
    }
    ...
    {
        $canBeBlocked = !in_array($this->status, [self::STATUS_BLOCKED, self::STATUS_DRAFT, self::STATUS_SENT]);

        return (bool) hooks()->applyFilters('campaign_can_be_blocked', $canBeBlocked, $this);
    }
    ...
    {
        return (string)$this->is_archived === self::TEXT_YES;
    }
    ...
    public function getSuppressionListsName(): string
    {
        ...
        foreach ($suppressionLists as $suppressionList) {
            $names[] = $suppressionList->suppressionList->name;
        }

        return $suppressionNames[$this->campaign_id] = implode(', ', $names);
    }
    ...
    public function getDeliveryServersNames(): string
    {
        ...
        foreach ($deliveryServers as $deliveryServer) {
            $names[] = sprintf('%s (%d)', $deliveryServer->getDisplayName(), $deliveryServer->server_id);
        }

        return $deliveryServersNames[$this->campaign_id] = implode(', ', $names);
    }

    /**
     * @return string
     */
    public function getAttachmentsDownloadLinks(): string
    {
        static $attachmentsDownloadLinks  = [];
        if (isset($attachmentsDownloadLinks[$this->campaign_id])) {
            return $attachmentsDownloadLinks[$this->campaign_id];
        }

        // assigned attachments
        $attachments = $this->attachments;
        if (empty($attachments)) {
            $attachmentsDownloadLinks[$this->campaign_id] = '';
        }

        $links = [];
        /** @var CampaignAttachment $attachment */
        foreach ($attachments as $attachment) {
            $links[] = CHtml::link($attachment->name, $attachment->getRelativePath() . $attachment->name, ['target' => '_blank', 'title' => t('app', 'Download')]);
        }

        return $attachmentsDownloadLinks[$this->campaign_id] = implode(', ', $links);
    }
    ...
    public function getTotalDeliveryTime(): string
    {
        ...
        $startedAt  = (int)strtotime((string)$this->started_at);
        $finishedAt = (int)strtotime((string)$this->finished_at);
        if ($startedAt === $finishedAt) {
            return $this->t('Less than a minute');
        }
    ...
    public function getBulkActionsList(): array
    {
        $actions = [
            ...
            self::BULK_ACTION_ARCHIVE_UNARCHIVE_CAMPAIGNS => t('app', 'Archive/Unarchive'),
        ];
     ...
     * @return void
     */
    public function _validateSendBetweenStart(string $attribute, array $params = []): void
    {
        if ($this->hasErrors($attribute)) {
            return;
        }

        if (empty($this->send_between_start) && !empty($this->send_between_interval)) {
            $this->addError('send_between_start', $this->t('{attr1} is required when {attr2} is provided.', [
                '{attr1}' => $this->getAttributeLabel('send_between_start'),
                '{attr2}' => $this->getAttributeLabel('send_between_interval'),
            ]));
        }
    }
     ...
     * @return void
     */
    public function _validateSendBetweenInterval(string $attribute, array $params = []): void
    {
        if ($this->hasErrors($attribute)) {
            return;
        }

        if (empty($this->send_between_interval) && !empty($this->send_between_start)) {
            $this->addError('send_between_interval', $this->t('{attr1} is required when {attr2} is provided.', [
                '{attr1}' => $this->getAttributeLabel('send_between_interval'),
                '{attr2}' => $this->getAttributeLabel('send_between_start'),
            ]));
        }
    }
     ...
     * @return bool
     */
    public function finishedSendingMoreThanXDaysAgo(int $days): bool
    {
        if (empty($this->finished_at)) {
            return false;
        }

        $finishedAt = \Carbon\Carbon::createFromTimestamp((int)strtotime((string)$this->finished_at));
        $daysAgo    = \Carbon\Carbon::createFromTimestamp((int)strtotime(sprintf('-%d days', $days)));

        return $finishedAt->lt($daysAgo);
    }
     ...
     * @return bool
     */
    public function finishedSendingMoreThanXHoursAgo(int $hours): bool
    {
        if (empty($this->finished_at)) {
            return false;
        }
        ...
        $finishedAt = \Carbon\Carbon::createFromTimestamp((int)strtotime((string)$this->finished_at));
        $hoursAgo    = \Carbon\Carbon::createFromTimestamp((int)strtotime(sprintf('-%d hours', $hours)));

        return $finishedAt->lt($hoursAgo);
    }
    ...
    public function getCanShowResetGiveupsButton(): bool
    {
        ...
        if ($this->finishedSendingMoreThanXDaysAgo(10)) {
            return false;
        }

        // do this way to avoid expensive count
        if (empty($this->option->giveup_count)) {
            return false;
        }

        // since 2.1.17
        // do this way to avoid expensive count since this button is rendered for each campaign in the grid
        // and would slow down things considerably if we would use a realtime counter
        if ($this->option->processed_count >= 0) {
            return false;
        }
    ...
    {
        return (int)$this->getCampaignDeliveryLogModel()->countByAttributes([
    ...
    public function postpone(): bool
    {
        ...
        // since 2.3.9
        $filterName = 'campaign_model_postpone_seconds_count';
        if (hooks()->hasFilters($filterName)) {
            $filterValue = (int) hooks()->applyFilters($filterName, 0, $this);
            $interval = sprintf('DATE_ADD(NOW(), INTERVAL %d SECOND)', $filterValue);
        }
    ...
    public function postponeBecauseCustomerReachedQuota(): bool
    {
        ...
        // each campaign has to have its own key connected to the quota mark so that
    ...
    {
        return !$this->getIsNewRecord() && ($this->getIsAutoresponder() || $this->getIsRegular());
    }
    ...
    public function updateAbTestSubjectOpensCountFromTrackOpen(CampaignTrackOpen $trackOpen): bool
    {
        ...
        try {
            ...
            $log = $this->getCampaignDeliveryLogModel()->find($criteria);
            if (empty($log)) {
                return false;
            }
     ...
     * @throws Exception
     */
    public function getDailySendingWindowDisplay(): string
    {
        $text = '';
        ...
        try {
            ...
            $start = Carbon\Carbon::createFromFormat('Y-m-d H:i:s', sprintf('%s %s', MW_DATE_NOW, $this->send_between_start));
            $startDisplay = $this->dateTimeFormatter->formatLocalizedTime($start->format('Y-m-d H:i:s'));

            $end = $start->addHours((int)$this->send_between_interval);
            $endDisplay = $this->dateTimeFormatter->formatLocalizedTime($end->format('Y-m-d H:i:s'));

            $text = $this->t('From {start} to {end}', [
                '{start}' => $startDisplay,
                '{end}' => $endDisplay,
            ]);
        } catch (Exception $e) {
     ...
     * @return string
     */
    public function getSendCampaignFirstLevelMutexKey(): string
    {
        return sprintf(
            'send-campaigns:campaign:%d:counter:%d',
            (int) $this->campaign_id,
            (int) $this->option->sending_from_processing_counter
        );
    }
     ...
     * @return string
     */
    public function getSendCampaignSecondLevelMutexKey(int $offset, int $limit): string
    {
        return sprintf(
            '%s:offset:%d:limit:%d',
            $this->getSendCampaignFirstLevelMutexKey(),
            $offset,
            $limit
        );
    }
     ...
     * @throws CException
     */
    public function hasProcessingActivity(): bool
    {
        ...
        /** @var CampaignDeliveryLog|null $lastDeliveryLog */
        $lastDeliveryLog = $this->getCampaignDeliveryLogModel()->find($criteria);

        if (empty($lastDeliveryLog)) {
            return false;
        }

        $allowedDeliveryLogTimeDifference = 300;

        $row = db()->createCommand(
            'SELECT MAX(`pause_after_send`) as pause_after_send FROM {{delivery_server}} WHERE `status` = :status'
        )->queryRow(true, [
            ':status' => DeliveryServer::STATUS_ACTIVE,
        ]);
        if (!empty($row) && !empty($row['pause_after_send'])) {
            $allowedDeliveryLogTimeDifference += round((int)$row['pause_after_send'] / 1000000, 6);
        }

        return strtotime((string)$lastDeliveryLog->date_added) + $allowedDeliveryLogTimeDifference >= time();
    }
     ...
     * @throws CException
     */
    public function resumeSendingFromProcessing(): bool
    {
        if (!$this->getIsProcessing()) {
            return false;
        }
        ...
        if (!$this->saveStatus(Campaign::STATUS_SENDING)) {
            return false;
        }

        $this->option->updateCounters([
            'sending_from_processing_counter' => 1,
        ], 'campaign_id = :cid', [
            ':cid' => (int)$this->campaign_id,
        ]);
     ...
     */
    public function getSmartTrackingService(): CampaignSmartTrackingService
    {
        return CampaignSmartTrackingService::createInstance($this);
    }
    ...
    protected function beforeSave()
    {
        ...
        // since 2.2.0
        if (empty($this->send_between_start)) {
            $this->send_between_start = null;
        }

        // since 2.2.3
        if (empty($this->send_between_interval)) {
            $this->send_between_interval = null;
        }
    ...
    protected function countSubscribersByListSegment(?CDbCriteria $mergeCriteria = null): int
    {
        ...
        return (int)$this->segment->countSubscribers($criteria, ['campaign_id' => $this->campaign_id]);
    }
    ...
    protected function findSubscribersByListSegment(
        ...
        return (array)$this->segment->findSubscribers($offset, $limit, $criteria, ['campaign_id' => $this->campaign_id]);
    }
    ...
    protected function countSubscribersByList(?CDbCriteria $mergeCriteria = null): int
    {
        ...
        return (int)ListSubscriber::model()->setListId((int)$this->list_id)->setCampaignId((int)$this->campaign_id)->count($criteria);
    }
    ...
    protected function findSubscribersByList(int $offset = 0, int $limit = 100, CDbCriteria $mergeCriteria = null): array
    {
        ...
        return (array)ListSubscriber::model()->setListId((int)$this->list_id)->setCampaignId((int)$this->campaign_id)->findAll($criteria);
    }
    ...
    protected function addAutoresponderCriteria(CDbCriteria $criteria): bool
    {
        ...
        } elseif ((string)$this->option->autoresponder_event === CampaignOption::AUTORESPONDER_EVENT_AFTER_PROFILE_UPDATE) {
            // All subscribers that update their profile AFTER the campaign starts sending.
            // This includes subscribers created before the campaign has been created.
            $where = [
                // subs that have the custom metadata
                '`customMetaData`.`key` = :cmdkey',
                'LENGTH(`customMetaData`.`value`) > 0',
                // and subs that updated their profile
                'CAST(`customMetaData`.`value` AS DATETIME) > t.date_added',
                // and did it after the campaign started sending
                'CAST(`customMetaData`.`value` AS DATETIME) >= :cdate',
            ];

            $criteria->params[':cmdkey'] = ListSubscriber::META_DATA_PROFILE_LAST_UPDATED_KEY_NAME;

            // However, in the case below, we only match subscribers that are added into the list AFTER the campaign starts sending.
            // We ignore subscribers added into the list BEFORE the campaign starts sending.
            if ($this->option->autoresponder_include_current != CampaignOption::TEXT_YES) {
                $where[] = 't.date_added >= :cdate';
            }

            $criteria->params[':cdate'] = $this->send_at;

            $criteria->with['customMetaData'] = [
                'select'   => false,
                'joinType' => 'INNER JOIN',
                'together' => true,
                'condition' => sprintf('(%s)', implode(' AND ', $where)),
            ];

            $condition = sprintf('DATE_ADD(CAST(`customMetaData`.`value` AS DATETIME), INTERVAL %d %s) <= NOW()', $timeValue, $timeUnit);
    ...
    protected function afterSave()
    {
        ...
        $this->setCampaignId($this->getCampaignId());
    }
    ...
    protected function instantiate($attributes)
    {
        ...
        $model->setCampaignId($this->getCampaignId());
