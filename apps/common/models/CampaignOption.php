 ...
 * @property string $smart_open_tracking
 * @property string $smart_click_tracking
 * @property string $open_tracking_from_url_tracking
 * @property string $open_tracking_exclude_crawlers
 * @property string $url_tracking_exclude_crawlers
 ...
 * @property int $sending_from_processing_counter
 *
...
class CampaignOption extends ActiveRecord
{
     ...
     */
    public const AUTORESPONDER_EVENT_AFTER_SUBSCRIBE = 'AFTER-SUBSCRIBE';
    public const AUTORESPONDER_EVENT_AFTER_PROFILE_UPDATE = 'AFTER-PROFILE-UPDATE';
    public const AUTORESPONDER_EVENT_AFTER_CAMPAIGN_OPEN = 'AFTER-CAMPAIGN-OPEN';
    public const AUTORESPONDER_EVENT_AFTER_CAMPAIGN_SENT = 'AFTER-CAMPAIGN-SENT';

    /**
     * Autoresponder time units
     */
    public const AUTORESPONDER_TIME_UNIT_MINUTE = 'minute';
    public const AUTORESPONDER_TIME_UNIT_HOUR   = 'hour';
    public const AUTORESPONDER_TIME_UNIT_DAY    = 'day';
    public const AUTORESPONDER_TIME_UNIT_WEEK   = 'week';
    public const AUTORESPONDER_TIME_UNIT_MONTH  = 'month';
    public const AUTORESPONDER_TIME_UNIT_YEAR   = 'year';
    ...
    {
        $rules = [
            ['url_tracking, open_tracking, smart_open_tracking, smart_click_tracking, open_tracking_from_url_tracking, open_tracking_exclude_crawlers, url_tracking_exclude_crawlers, json_feed, xml_feed, embed_images, plain_text_email', 'required'],
            ['url_tracking, open_tracking, smart_open_tracking, smart_click_tracking, open_tracking_from_url_tracking, open_tracking_exclude_crawlers, url_tracking_exclude_crawlers, json_feed, xml_feed, embed_images, plain_text_email', 'length', 'max' => 3],
            ['url_tracking, open_tracking, smart_open_tracking, smart_click_tracking, open_tracking_from_url_tracking, open_tracking_exclude_crawlers, url_tracking_exclude_crawlers, json_feed, xml_feed, embed_images, plain_text_email', 'in', 'range' => array_keys($this->getYesNoOptions())],
            ...
            ['autoresponder_time_value', 'numerical', 'integerOnly' => true, 'min' => 0, 'max' => 50000],
    ...
    public function attributeLabels()
    {
        $labels = [
            ...
            'smart_open_tracking'             => t('campaigns', 'Smart open tracking'),
            'smart_click_tracking'            => t('campaigns', 'Smart click tracking'),
            'open_tracking_from_url_tracking' => t('campaigns', 'Open tracking from url tracking'),
            'open_tracking_exclude_crawlers'  => t('campaigns', 'Open tracking - Exclude crawlers'),
            'url_tracking_exclude_crawlers'   => t('campaigns', 'Url tracking - Exclude crawlers'),
    ...
    {
        $texts = [
            'url_tracking'                      => t('campaigns', 'Whether to enable url tracking. If your click rates are too high, you might try to set the "Url tracking - Exclude crawlers" option to Yes'),
            'open_tracking'                     => t('campaigns', 'Whether to enable opens tracking. If your open rates are too high, you might try to disable this option and use the "Open tracking from url tracking" option, and/or set the "Open tracking - Exclude crawlers" option to Yes'),
            'smart_open_tracking'               => t('campaigns', 'Whether to enable smart opens tracking. The system will learn based on various rule sets and will not record actions matching these rules. Works only when open tracking is enabled.'),
            'smart_click_tracking'              => t('campaigns', 'Whether to enable smart click tracking. The system will learn based on various rule sets and will not record actions matching these rules. Works only when click tracking is enabled.'),
            'open_tracking_from_url_tracking'   => t('campaigns', 'In case open tracking is disabled, or the open tracking pixel cannot be loaded, we can try to record the open when one of the links inside the campaign is clicked. This only works if url tracking is enabled. We only track unique opens this way, one open per subscriber'),
            'open_tracking_exclude_crawlers'    => t('campaigns', 'Whether to exclude crawlers/bots/proxies when tracking opens. This will try to record opens done by the subscribers themselves, which mean your open rates will be much lower than usual, but most likely more accurate'),
            'url_tracking_exclude_crawlers'     => t('campaigns', 'Whether to exclude crawlers/bots/proxies when tracking urls. This will try to record clicks done by the subscribers themselves, which mean your click rates will be much lower than usual, but most likely more accurate'),
    ...
    public function getAutoresponderEvents(): array
    {
        return [
            ...
            self::AUTORESPONDER_EVENT_AFTER_PROFILE_UPDATE  => t('campaigns', self::AUTORESPONDER_EVENT_AFTER_PROFILE_UPDATE),
     ...
     * @return bool
     */
    public function getSmartClickTracking(): bool
    {
        return $this->url_tracking === self::TEXT_YES && $this->smart_click_tracking === self::TEXT_YES;
    }

    /**
     * @return bool
     */
    public function getSmartOpenTracking(): bool
    {
        return $this->open_tracking === self::TEXT_YES && $this->smart_open_tracking === self::TEXT_YES;
    }
    ...
    {
        if ($this->url_tracking === self::TEXT_NO) {
            $this->smart_click_tracking = self::TEXT_NO;
        }

        if ($this->open_tracking === self::TEXT_NO) {
            $this->smart_open_tracking = self::TEXT_NO;
        }
