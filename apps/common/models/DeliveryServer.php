 ...
 * @property integer $second_quota
 * @property integer $minute_quota
 ...
 * @property string|int $use_for
 * @property string $signing_enabled
 * @property string $force_from
 * @property string $force_from_name
...
class DeliveryServer extends ActiveRecord
{
    ...
    public const TRANSPORT_SMTP_AMAZON = 'smtp-amazon';
    public const TRANSPORT_SMTP_POSTMASTERY = 'smtp-postmastery';
    public const TRANSPORT_SMTP_GREENARROW = 'smtp-greenarrow';
    public const TRANSPORT_SMTP_POSTAL = 'smtp-postal';
    public const TRANSPORT_SMTP_MYSMTPCOM = 'smtp-mysmtpcom';
    public const TRANSPORT_SMTP_PMTA = 'smtp-pmta';
    public const TRANSPORT_SMTP_INBOXROAD = 'smtp-inboxroad';
    public const TRANSPORT_SENDMAIL = 'sendmail';
    public const TRANSPORT_PICKUP_DIRECTORY = 'pickup-directory';
    public const TRANSPORT_AMAZON_SES_WEB_API = 'amazon-ses-web-api';
    public const TRANSPORT_MAILGUN_WEB_API = 'mailgun-web-api';
    public const TRANSPORT_SENDGRID_WEB_API = 'sendgrid-web-api';
    public const TRANSPORT_INBOXROAD_WEB_API = 'inboxroad-web-api';
    public const TRANSPORT_ELASTICEMAIL_WEB_API = 'elasticemail-web-api';
    public const TRANSPORT_SPARKPOST_WEB_API = 'sparkpost-web-api';
    public const TRANSPORT_PEPIPOST_WEB_API = 'pepipost-web-api';
    public const TRANSPORT_POSTMARK_WEB_API = 'postmark-web-api';
    public const TRANSPORT_MAILJET_WEB_API = 'mailjet-web-api';
    public const TRANSPORT_MAILERQ_WEB_API = 'mailerq-web-api';
    public const TRANSPORT_SENDINBLUE_WEB_API = 'sendinblue-web-api';
    public const TRANSPORT_BREVO_WEB_API = 'brevo-web-api';
    public const TRANSPORT_TIPIMAIL_WEB_API = 'tipimail-web-api';
    public const TRANSPORT_NEWSMAN_WEB_API = 'newsman-web-api';
    public const TRANSPORT_POSTAL_WEB_API = 'postal-web-api';

    /**
     * Delivery flags
     */
    public const DELIVERY_FOR_SYSTEM = 'system';
    ...
    public const DELIVERY_FOR_LIST = 'list';
    public const DELIVERY_FOR_TRANSACTIONAL = 'transactional';

    /**
     * Usage Flags
     */
    public const USE_FOR_ALL = 1;
    public const USE_FOR_TRANSACTIONAL = 2;
    public const USE_FOR_CAMPAIGNS = 4;
    public const USE_FOR_EMAIL_TESTS = 8;
    public const USE_FOR_REPORTS = 16;
    public const USE_FOR_LIST_EMAILS = 32;
    public const USE_FOR_INVOICES = 64;
     ...
     * Various force flags
     */
    public const FORCE_FROM_WHEN_NO_SIGNING_DOMAIN = 'when no valid signing domain';
    public const FORCE_FROM_ALWAYS = 'always';
    public const FORCE_FROM_NEVER = 'never';
    public const FORCE_REPLY_TO_ALWAYS = 'always';
    public const FORCE_REPLY_TO_NEVER = 'never';
    public const FORCE_FROM_NAME_ALWAYS = 'always';
    public const FORCE_FROM_NAME_NEVER = 'never';

    /**
     * Cache flags
     */
    public const QUOTA_CACHE_SECONDS = 300;
     ...
     * @var string
     */
    protected $_secondQuotaAccessKey = 'DeliveryServerGetSecondQuotaLeft::%d';
     ...
     * @var int
     */
    protected $_initSecondQuota  = 0;

    /**
     * @var int
     */
    protected $_initMinuteQuota  = 0;
    ...
    public function rules()
    {
        $rules = [
            ...
            ['second_quota, minute_quota, hourly_quota, daily_quota, monthly_quota, pause_after_send', 'numerical', 'integerOnly' => true, 'min' => 0],
            ['second_quota, minute_quota, hourly_quota, daily_quota, monthly_quota, pause_after_send', 'length', 'max' => 11],
            ...
            ['multi_use_for', '_validateMultiUseFor'],
            ['force_from', 'in', 'range' => array_keys($this->getForceFromOptions())],
            ['force_from_name', 'in', 'range' => array_keys($this->getForceFromNameOptions())],
    ...
    public function attributeLabels()
    {
        $labels = [
            ...
            'second_quota'                  => t('servers', 'Second quota'),
            'minute_quota'                  => t('servers', 'Minute quota'),
            ...
            'force_from'                    => t('servers', 'Force FROM email'),
            'force_from_name'               => t('servers', 'Force FROM name'),
            ...
            'multi_use_for'                   => t('servers', 'Use for'),
        ];
    ...
    public function attributeHelpTexts()
    {
        $texts = [
            ...
            'warmup_plan_id'            => t('servers', 'The warmup plan that is attached to this delivery server. The quota of this server will be calculated based on the warmup plan. This is an autocomplete field, start typing the plan name to get suggestions.'),
            ...
            'second_quota'              => t('servers', 'In case there are limits that apply for sending with this server, you can set a second quota for it and it will only send in one second as many emails as you set here. Set it to 0 in order to not apply any second limit.'),
            'minute_quota'              => t('servers', 'In case there are limits that apply for sending with this server, you can set a minute quota for it and it will only send in one minute as many emails as you set here. Set it to 0 in order to not apply any minute limit.'),
            ...
            'force_from_name'           => t('servers', 'When to force the FROM name'),
        ...
        $texts['multi_use_for'] = $texts['use_for'];
    ...
    {
        trigger_error(sprintf('The method %s is deprecated, use %s::send instead!', __METHOD__, self::class), E_USER_DEPRECATED);
    ...
    public function getBounceServersArray(): array
    {
        ...
        foreach ($models as $model) {
            $_options[(string)$model->server_id] = $model->getDisplayName();
        }
    ...
    public function getSigningSupportedTypes(): array
    {
        $types = [
            ...
            self::TRANSPORT_SMTP_GREENARROW,
    ...
    public function getTrackingDomainsArray(): array
    {
        ...
        $_options[''] = t('app', 'Choose');
        foreach ($models as $model) {
            $_options[(string)$model->domain_id] = $model->name;
        }
    ...
    public function getParamsArray(array $params = []): array
    {
        ...
        $defaultParams = CMap::mergeArray([
            ...
            'forcedFromEmail'       => '',
            'forceFromName'         => (string)$this->force_from_name,
            'forcedFromName'        => '',
        ...
        // since 2.3.8
        if ($params['forceFromName'] == self::FORCE_FROM_NAME_ALWAYS) {
            $params['forcedFromName'] = $this->from_name;
            $params['fromName'] = $this->from_name;
            $params['fromNameCustom'] = $this->from_name;
            if (is_array($params['from']) && count($params['from']) == 1) {
                foreach ($params['from'] as $email => $value) {
                    $params['from'][$email] = $this->from_name;
                }
            }
            if (is_array($params['sender']) && count($params['sender']) == 1) {
                foreach ($params['sender'] as $email => $value) {
                    $params['sender'][$email] = $this->from_name;
                }
            }
        }
        //
        ...
        return (array)hooks()->applyFilters('delivery_server_get_params_array', $params, new CAttributeCollection([
    ...
    public static function getTypesMapping(): array
    {
        ...
        $mapping = [
            ...
            self::TRANSPORT_BREVO_WEB_API        => DeliveryServerBrevoWebApi::class,
            ...
            self::TRANSPORT_SMTP_GREENARROW      => DeliveryServerSmtpGreenArrow::class,
    ...
    public function logUsage(): ?DeliveryServerUsageLog
    {
        ...
        $this->decreaseSecondQuota();
        $this->decreaseMinuteQuota();
    ...
    public function undoLogUsage(DeliveryServerUsageLog $log): bool
    {
        ...
        $this->decreaseSecondQuota(-1);
        $this->decreaseMinuteQuota(-1);
     ...
     * @return int
     */
    public function countSecondUsage(): int
    {
        $count = 0;
        try {
            $criteria = new CDbCriteria();
            ...
            $criteria->addCondition('
                `date_added` BETWEEN DATE_FORMAT(NOW() - INTERVAL 1 SECOND, "%Y-%m-%d %H:%i:%s") AND 
     ...
     * @return bool
     */
    public function getCanHaveSecondQuota(): bool
    {
        return !$this->getIsNewRecord() && $this->second_quota > 0;
    }

    /**
     * @return int
     */
    public function countMinuteUsage(): int
    {
        $count = 0;
        try {
            $criteria = new CDbCriteria();
            $criteria->compare('server_id', (int)$this->server_id);
            $criteria->addCondition('
                `date_added` BETWEEN DATE_FORMAT(NOW(), "%Y-%m-%d %H:%i:00") AND 
     ...
     * @return bool
     */
    public function getCanHaveMinuteQuota(): bool
    {
        return !$this->getIsNewRecord() && $this->minute_quota > 0;
    }
     ...
     * @return int
     */
    public function getSecondQuotaLeft(bool $useMutex = true): int
    {
        if (!$this->getCanHaveSecondQuota()) {
            return PHP_INT_MAX;
        }

        $accessKey = sha1(sprintf($this->_secondQuotaAccessKey, (int)$this->server_id));
        ...
        $sendingsLeft = $this->second_quota - $this->countSecondUsage();
        $sendingsLeft = max($sendingsLeft, 0);

        cache()->set($accessKey, $sendingsLeft, 1);
        if ($useMutex) {
            mutex()->release($accessKey);
        }
     ...
     * @return int
     */
    public function decreaseSecondQuota(int $by = 1, bool $useMutex = true): int
    {
        if (!$this->getCanHaveSecondQuota()) {
            return PHP_INT_MAX;
        }
        ...
        $sendingsLeft = $this->getSecondQuotaLeft(!$useMutex) - (int)$by;
        $sendingsLeft = max($sendingsLeft, 0);
    ...
    {
        if (!$this->getCanHaveMinuteQuota()) {
            return PHP_INT_MAX;
        }

        $accessKey = sha1(sprintf($this->_minuteQuotaAccessKey, (int)$this->server_id));
        ...
        $sendingsLeft = $this->minute_quota - $this->countMinuteUsage();
        $sendingsLeft = max($sendingsLeft, 0);

        cache()->set($accessKey, $sendingsLeft, 30);
        if ($useMutex) {
            mutex()->release($accessKey);
        }
    ...
    public function decreaseMinuteQuota(int $by = 1, bool $useMutex = true): int
    {
        ...
        $sendingsLeft = $this->getMinuteQuotaLeft(!$useMutex) - (int)$by;
        $sendingsLeft = max($sendingsLeft, 0);

        cache()->set($accessKey, $sendingsLeft, 60);
    ...
    public function getCanHaveQuota(): bool
    {
        ...
        return $this->getCanHaveSecondQuota() ||
               $this->getCanHaveMinuteQuota() ||
               $this->getCanHaveHourlyQuota() ||
               $this->getCanHaveDailyQuota() ||
               $this->getCanHaveMonthlyQuota();
    }
    ...
    public function getIsOverQuota(): bool
    {
        ...
        if ($this->getSecondQuotaLeft() == 0) {
            return true;
        }

        if ($this->getMinuteQuotaLeft() == 0) {
            return true;
        }
     ...
     * @param array $params
     */
    public function _validateMultiUseFor(string $attribute, array $params = []): void
    {
        $this->use_for = 0;

        $multiUseFor = $this->$attribute;
        if (empty($multiUseFor)) {
            return;
        }

        if (!is_array($multiUseFor)) {
            $this->addError($attribute, t('servers', 'This attribute must be an array'));
            return;
        }

        foreach ($multiUseFor as $value) {
            if (!isset($this->getUseForOptions()[$value])) {
                $this->addError($attribute, t('servers', 'Invalid selection'));
                return;
            }
        }

        foreach ($multiUseFor as $value) {
            $this->use_for |= (int)$value;
        }
    }
    ...
    {
        $hookParams = new CAttributeCollection([
            'currentServerId'   => $currentServerId,
            'deliveryObject'    => $deliveryObject,
            'params'            => $params,
        ]);

        $server = hooks()->applyFilters('delivery_servers_before_pick_server', null, $hookParams);
        if ($server instanceof DeliveryServer) {
            return $server;
        }

        /** @var DeliveryServer|null $server */
        $server = self::handlePickServer($currentServerId, $deliveryObject, $params);

        $server = hooks()->applyFilters('delivery_servers_after_pick_server', $server, $hookParams);

        return $server instanceof DeliveryServer ? $server : null;
    }
    ...
    public function copy(): ?self
    {
        ...
        try {
            /** @var DeliveryServer $server */
            $server = $this->createNewInstanceFromLoadedAttributes();
            ...
            $server->additional_headers = $this->additional_headers;
            ...
            if (!empty($this->domainPolicies)) {
                foreach ($this->domainPolicies as $policy) {
                    ...
                    $policy = $policy->createNewInstanceFromLoadedAttributes();
     ...
     * @return array
     */
    public function getForceFromNameOptions(): array
    {
        return [
            self::FORCE_FROM_NAME_NEVER => ucfirst(t('servers', self::FORCE_FROM_NAME_NEVER)),
            self::FORCE_FROM_NAME_ALWAYS => ucfirst(t('servers', self::FORCE_FROM_NAME_ALWAYS)),
        ];
    }
    ...
    {
        return [
            self::USE_FOR_ALL           => t('servers', 'All'),
            self::USE_FOR_CAMPAIGNS     => t('servers', 'Campaigns'),
            self::USE_FOR_TRANSACTIONAL => t('servers', 'Transactional emails'),
     ...
     * @return bool
     */
    public function getUseFor(int $for): bool
    {
        return (int)$this->use_for === self::USE_FOR_ALL || ($for & (int)$this->use_for);
    }
    ...
    {   // Use tracking only for active delivery servers, so when validating one, the confirmation url will not use the tracking domain, which would force the user to login again
        return $this->getIsActive() && !empty($this->tracking_domain_id) && !empty($this->trackingDomain) && !empty($this->trackingDomain->name);
    }
    ...
    public function getFormFieldsDefinition(array $fields = []): array
    {
        ...
        $defaults = [
            ...
            'second_quota' => [
                'visible'   => true,
                'fieldHtml' => $form->numberField($this, 'second_quota', $this->fieldDecorator->getHtmlOptions('second_quota', [
            ...
            'minute_quota' => [
                'visible'   => true,
                'fieldHtml' => $form->numberField($this, 'minute_quota', $this->fieldDecorator->getHtmlOptions('minute_quota', [
                    'disabled' => !empty($this->warmup_plan_id) && !empty($this->warmupPlan) && !$this->warmupPlan->getIsDeliveryServerCompleted((int)$this->server_id),
                    'readonly' => !empty($this->warmup_plan_id) && !empty($this->warmupPlan) && !$this->warmupPlan->getIsDeliveryServerCompleted((int)$this->server_id),
                    'class'    => 'form-control has-help-text delivery-server-quota-form-field',
                ])),
            ...
            'multi_use_for' => [
                'visible'   => true,
                'fieldHtml' => $form->dropDownList($this, 'multi_use_for', $this->getUseForOptions(), $this->fieldDecorator->getHtmlOptions('use_for', [
                    'multiple'      => true,
                    'unselectValue' => self::USE_FOR_ALL,
                    'class'         => 'form-control has-help-text select2',
                ])),
            ],
            'signing_enabled' => [
                'visible'    => true,
            ...
            'force_from_name' => [
                'visible'   => true,
                'fieldHtml' => $form->dropDownList($this, 'force_from_name', $this->getForceFromNameOptions(), $this->fieldDecorator->getHtmlOptions('force_from_name')),
            ],
            'force_sender' => [
                'visible'    => true,
     ...
     * @throws CException
     */
    protected static function handlePickServer(int $currentServerId = 0, $deliveryObject = null, array $params = []): ?self
    {
        // since 1.3.6.3
        ...
        if ($customer = self::parseDeliveryObjectForCustomer($deliveryObject)) {
            ...
            $serverIds = [];
            $criteria  = new CDbCriteria();
            $criteria->select = 't.server_id, t.monthly_quota, t.daily_quota, t.hourly_quota, t.minute_quota, t.second_quota';
            ...
            if (!empty($customer->group_id)) {
                ...
                if (!empty($groupServers[$customer->group_id])) {
                    ...
                    $servers   = self::model()->findAll([
                        'select'    => 't.server_id, t.monthly_quota, t.daily_quota, t.hourly_quota, t.minute_quota, t.second_quota',
    ...
    protected function afterConstruct()
    {
        ...
        // since 2.3.8
        $this->force_from_name = self::FORCE_FROM_NAME_NEVER;

        // 2.3.9
        $this->multi_use_for = [self::USE_FOR_ALL];
    ...
    protected function afterFind()
    {
        ...
        // 2.3.8
        $this->multi_use_for = array_filter(array_keys($this->getUseForOptions()), function ($item): bool {
            return (int)$item && ((int)$item & (int)$this->use_for);
        });
        //

        // since 1.5.0
        $this->_initSecondQuota  = $this->second_quota;
        $this->_initMinuteQuota  = $this->minute_quota;
    ...
    protected function afterValidate()
    {
        if (!$this->getIsNewRecord() && !is_cli()) {
            ...
            foreach ($keys as $key) {
                if ((string)$model->$key !== (string)$this->$key) {
    ...
    protected function afterSave()
    {
        ...
        // since 1.5.0
        if ((int)$this->second_quota != (int)$this->_initSecondQuota) {
            cache()->delete(sha1(sprintf($this->_secondQuotaAccessKey, (int)$this->server_id)));
        }
        if ((int)$this->minute_quota != (int)$this->_initMinuteQuota) {
            cache()->delete(sha1(sprintf($this->_minuteQuotaAccessKey, (int)$this->server_id)));
        }
    ...
    protected static function processPickServerCriteria(CDbCriteria $criteria, int $currentServerId = 0, $deliveryObject = null, array $params = []): array
    {
        ...
        if ($campaign_id > 0 && !isset($campaignServers[$campaign_id])) {
            ...
            if (!empty($_serverIds)) {
                ...
                $_criteria->select = 't.server_id, t.second_quota, t.minute_quota, t.hourly_quota, t.daily_quota, t.monthly_quota';
        ...
        // since 2.3.6
        if (!empty($params['useFor']) && !is_array($params['useFor'])) {
            $params['useFor'] = [(int)$params['useFor']];
        }

        // since 1.3.5
        if (!empty($params['useFor']) && !in_array(self::USE_FOR_ALL, $params['useFor'])) {
            $params['useFor'][] = self::USE_FOR_ALL;
        }
        //

        // since 2.3.8
        if (!empty($params['useFor'])) {
            $useFor = 0;
            foreach ($params['useFor'] as $value) {
                if (!isset(self::model()->getUseForOptions()[$value])) {
                    continue;
                }
                $useFor |= (int)$value;
            }
            $_criteria->addCondition(sprintf('(t.use_for <= %d OR (%d & t.use_for > 0))', self::USE_FOR_ALL, $useFor));
        }
        ...
        $probability = random_int((int)$lowest, 100);
