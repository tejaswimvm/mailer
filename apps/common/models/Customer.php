...
class Customer extends ActiveRecord
{
     ...
     * Statuses list
     */
    public const STATUS_PENDING_CONFIRM = 'pending-confirm';
    public const STATUS_PENDING_ACTIVE = 'pending-active';
    public const STATUS_PENDING_DELETE = 'pending-delete';
    public const STATUS_PENDING_DISABLE = 'pending-disable';
    ...
    public function getAvailableDeliveryServers(): array
    {
        ...
        // since 1.3.5
        $criteria->addCondition(sprintf('(use_for <= %d OR (%d & use_for > 0))', DeliveryServer::USE_FOR_ALL, DeliveryServer::USE_FOR_CAMPAIGNS));
    ...
    public function countHourlyUsage(): int
    {
        ...
        if (!mutex()->acquire($cacheKey, 60)) {
            Yii::log(sprintf('%s unable to acquire the mutex after 60 seconds!', __METHOD__), CLogger::LEVEL_ERROR);
            return PHP_INT_MAX;
        }
    ...
    public function getSendingQuotaUsageDisplay(): string
    {
        if (!empty($this->parent_id) && !empty($this->parent)) {
            return $this->parent->getSendingQuotaUsageDisplay();
        }
    ...
    public function isOverPricePlanLimits(PricePlan $pricePlan): CAttributeCollection
    {
        ...
        $limit = (int)$grp->getOptionValue($kp . 'tracking_domains.max_tracking_domains', 0);
        if ($limit > 0) {
            $in->add('limit', $limit);
            $in->add('count', (int)TrackingDomain::model()->countByAttributes(['customer_id' => (int)$this->customer_id]));
            $in->add('object', 'tracking domains');
            if ((int)$in->itemAt('count') > (int)$in->itemAt('limit')) {
                return $in;
            }
        }
