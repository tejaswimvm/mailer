...
{
    public const DEFAULT_CATEGORY = 'misc';
    ...
    public function set(string $key, $value): self
    {
        ...
        if ($this->get($_key) !== null) {
            $command->update('{{option}}', [
                'value'         => is_string($value) ? $value : SerializeHelper::serialize($value),
    ...
    protected function loadCategory(string $key): self
    {
        ...
        foreach ($rows as $row) {
            $this->_options[$row['category'] . '.' . $row['key']] = !$row['is_serialized'] ? $row['value'] : SerializeHelper::unserialize($row['value']);
        }
