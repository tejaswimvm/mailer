...
class CActiveFinder extends CComponent
{
     ...
     * @return CJoinElement
     */
    public function getJoinTree(): CJoinElement
    {
        ...
        if (is_string($with)) {
            ...
            if (($relation = $parent->model->getActiveRelation($with)) === null) {
                throw new CDbException(Yii::t(
                    ...
                    ['{class}' => get_class($parent->model), '{name}' => $with]
                ));
            }
            ...
            if (!empty($relation->scopes)) {
                $scopes = array_merge($scopes, (array) $relation->scopes);
            } // no need for complex merging

            if (!empty($options['scopes'])) {
                $scopes = array_merge($scopes, (array) $options['scopes']);
            } // no need for complex merging
...
class CJoinElement
{
    ...
    public function count($criteria = null)
    {
        ...
        if ($select !== '*' && preg_match('/^count\s*\(/i', trim((string)$select))) {
        ...
        if ($this->relation instanceof CManyManyRelation) {
            ...
            foreach ($fks as $i => $fk) {
                if (isset($joinTable->foreignKeys[$fk])) {  // FK defined
                    [$tableName, $pk] = $joinTable->foreignKeys[$fk];
    ...
    private function queryOneMany()
    {
        ...
        foreach ($fks as $i => $fk) {
            ...
            if (isset($table->foreignKeys[$fk])) {
                [$tableName, $pk] = $table->foreignKeys[$fk];
