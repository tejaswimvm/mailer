...
class CampaignXmlFeedParser
{
    ...
    public static function parseContent(
        ...
        // since 2.3.9 check if the feed url is dynamic, and if it is, don't use cache at this level
        $isDynamicFeedURL = false;
        foreach ($multiMatches[0] as $fullFeedHtml) {
            ...
            $isDynamicFeedURL = (bool)preg_match('/\[([A-Z0-9_]+)]/', $feedStartTag);
        }
        //
        ...
        if (!$isDynamicFeedURL && $cache && ($cachedContent = cache()->get($cacheKey))) {
        ...
        foreach ($multiMatches[0] as $fullFeedHtml) {
            ...
            $feedItemsMap = [
                ...
                '[XML_FEED_ITEM_CREATOR]'       => 'creator',
            ];

            $html = '';

            // since 2.1.20 - start - handle row based items first
            $htmlAppended = false;
            $feedItemTemplateHash = sha1($feedItemTemplate);
            foreach ($feedItems as $index => $feedItem) {
                foreach ($feedItemsMap as $tag => $mapValue) {
                    ...
                    $feedItemTemplate   = (string)str_replace($tagNum, $feedItem[$mapValue], $feedItemTemplate);
                }
            }
            $feedItemTemplateChanged = $feedItemTemplateHash != sha1($feedItemTemplate);
            // since 2.1.20 - end
            ...
            // since 2.1.20 - start
            if ($feedItemTemplateChanged && !$htmlAppended) {
                $html .= $feedItemTemplate;
            }
            // since 2.1.20 - end
    ...
    public static function getRemoteFeedItems(
        ...
        $accessKey = sha1(sprintf('m:%s.c:%s.u:%s.c:%s.o:%s.d:%s.n:%s.s:%s', __METHOD__, $campaign->campaign_uid, $url, $count, $offset, $daysBack, $noItemAction, (string)$campaign->send_at));
        ...
        if (empty($xml)) {
            ...
            if ($noItemAction == 'postpone-campaign' && $campaign->getIsProcessing()) {
                // since 2.3.2
                $hours = (int)hooks()->applyFilters('campaign_feed_parser_no_items_postpone_campaign_hours_count', 24);
                $campaign->saveSendAt(new CDbExpression(sprintf('DATE_ADD(NOW(), INTERVAL %d HOUR)', $hours)));
                mutex()->release($accessKey);
        ...
        foreach ($xml->channel->item as $item) {
            ...
            if (!empty($dc) && isset($dc->creator)) {
                $itemMap['creator'] = (string)$dc->creator;
            }

            $itemMap = array_map('html_decode', array_map('strval', $itemMap));

            // 1.7.4
            if ($sendOnlyUniqueItems) {
                // make sure the item hasn't been served. If it has, skip it
                $itemCacheKey = (int)$campaign->list_id . ':' . (int)$campaign->segment_id . ':' . sha1(SerializeHelper::serialize($itemMap));
