...
class StringHelper
{
    ...
    /**
     * Check issue #1122 before adjusting this method.
     * We decided that we should not try to fix broken urls that don't follow standards.
     * Thus, we only replace what we consider to be safe.
     *
     * @param string $url
     *
     * @return string
    ...
    public static function normalizeUrl(string $url): string
    {
        ...
        if (strpos($url, '?') !== false || strpos($url, '&') !== false) {
            ...
            $url = (string)str_ireplace(array_keys($replacements), array_values($replacements), $url);
        }
        return $url;
    }
    ...
    public static function removeComparisonSignsPrefix(string $string): string
    {
        ...
        foreach ($signs as $sign) {
            ...
            $variations[] = preg_quote(html_encode((string)$sign), '/');
        }
