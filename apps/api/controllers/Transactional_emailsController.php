...
class Transactional_emailsController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $maxPerPage = (int)hooks()->applyFilters('api_transactional_emails_collection_max_records_per_page', 1000);
        $minPerPage = (int)hooks()->applyFilters('api_transactional_emails_collection_min_records_per_page', 10);
        ...
        $criteria = hooks()->applyFilters('api_transactional_emails_collection_count_criteria', $criteria);
        ...
        /** @var CDbCriteria $criteria */
        $criteria = hooks()->applyFilters('api_transactional_emails_collection_find_criteria', $criteria);
        ...
        foreach ($emails as $email) {
            ...
            $data['records'][] = hooks()->applyFilters('api_transactional_emails_collection_record', $attributes, $email);
        }

        $this->renderJson([
            'status' => 'success',
            'data'   => hooks()->applyFilters('api_transactional_emails_collection_data', $data),
        ]);
    }
