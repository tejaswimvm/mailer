...
class CampaignsController extends Controller
{
    ...
    public function actionWeb_version($campaign_uid, $subscriber_uid = null)
    {
        ...
        if ($subscriber) {
            ...
            $log = $campaign->getCampaignDeliveryLogModel()->findByAttributes([
        ...
        if (CampaignHelper::hasRemoteContentTag($emailContent)) {
            $emailContent = CampaignHelper::fetchContentForRemoteContentTag($emailContent, $campaign, $subscriber, $server);
        }
        //

        // 2.1.15
        if (CampaignHelper::hasCountdownTag($emailContent)) {
        ...
        if ($subscriber) {
            if (!$campaign->getIsDraft() && !empty($campaign->option) && $campaign->option->url_tracking == CampaignOption::TEXT_YES) {
                $emailContent = CampaignHelper::transformLinksForTracking($emailContent, $campaign, $subscriber, false, false, false);
            }
        } else {
            $subscriber = new ListSubscriber();
        }

        $emailData = CampaignHelper::parseContent($emailContent, $campaign, $subscriber, true, $server);
        ...
        if (
            ...
            $emailContent = CampaignHelper::transformLinksForTracking($emailContent, $campaign, $subscriber, true, false, false);
    ...
    public function actionTrack_opening($campaign_uid, $subscriber_uid)
    {
        ...
        // since 2.1.15
        if ($campaign->option->open_tracking_exclude_crawlers == CampaignOption::TEXT_YES) {
            $detect = new Jaybizzle\CrawlerDetect\CrawlerDetect(null, request()->getUserAgent());
            if ($detect->isCrawler()) {
                if ($this->trackOpeningInternalCall) {
                    return;
                }
        ...
        // since 2.4.9
        if ($canTrack &&
            !$this->trackOpeningInternalCall &&
            !$campaign->getSmartTrackingService()->isValidRequest(
                (string)request()->getUserHostAddress(),
                CampaignTrackingIgnoreList::ACTION_OPEN,
                $subscriber
            )
        ) {
            $canTrack = false;
    ...
    public function actionTrack_url($campaign_uid, $subscriber_uid, $hash)
    {
        ...
        // since 2.1.15
        if ($campaign->option->url_tracking_exclude_crawlers == CampaignOption::TEXT_YES) {
            $detect = new Jaybizzle\CrawlerDetect\CrawlerDetect(null, request()->getUserAgent());
            if ($detect->isCrawler()) {
                app()->end();
                return;
        ...
        /** @var OptionUrl $optionsUrl */
        $optionsUrl = container()->get(OptionUrl::class);

        // since 2.2.9
        $appHostInfo     = rtrim((string)str_replace('index.php', '', $optionsUrl->getFrontendUrl()), '/');
        $currentHostInfo = request()->getHostInfo();

        // since 2.2.14
        // $currentHostInfo does not include the path, so if the app is installed in a subfolder, we need to take it into consideration.
        // $appHostInfo does not include a trailing slash.
        // $appHostPath will start with a trailing slash if there is a subfolder
        $appHostPath = parse_url($appHostInfo, PHP_URL_PATH);
        if (!empty($appHostPath)) {
            $currentHostInfo .= $appHostPath;
        }
        // end 2.2.14 fix

        $usingCname = !empty($appHostInfo) && !empty($currentHostInfo) && !(stripos($appHostInfo, $currentHostInfo) === 0);
        ...
        if ($canTrack &&
            ...
            !$campaign->getSmartTrackingService()->isValidRequest(
                (string)request()->getUserHostAddress(),
                CampaignTrackingIgnoreList::ACTION_CLICK,
                $subscriber
            )
        ) {
            $canTrack = false;
        ...
        if (!$canTrack) {
            ...
            // since 2.2.9
            // if we're under a cname, and we have to redirect to an url of our application,
            // then make sure we use the cname instead of the application domain.
            // this generally happens when using tracking domains
            if ($usingCname && stripos($destination, $appHostInfo) === 0) {
                $destination = (string)str_replace($appHostInfo, $currentHostInfo, $destination);
            }
        ...
        // since 1.7.6
        if (!empty($destination) && CampaignHelper::isTemplateEngineEnabled()) {
            $searchReplace = CampaignHelper::getCommonTagsSearchReplace($destination, $campaign, $subscriber, $server);
            $destination   = CampaignHelper::parseByTemplateEngine($destination, $searchReplace);
            $destination   = StringHelper::normalizeUrl($destination);
        }
        //

        if ($campaign->option->open_tracking_from_url_tracking == CampaignOption::TEXT_YES && !$subscriber->hasOpenedCampaign($campaign)) {
    ...
    /**
     * Will catch the clicks made by the bots and record them in an ignore list for tracking
     *
     * @param string $campaign_uid
     * @param string $subscriber_uid
     *
     ...
     * @throws CHttpException
     */
    public function actionTracking_ignore_click_url($campaign_uid, $subscriber_uid)
    {
        $criteria = new CDbCriteria();
        $criteria->compare('campaign_uid', $campaign_uid);
        $criteria->addNotInCondition('status', [Campaign::STATUS_PENDING_DELETE]);
        ...
        $ignored->action     = CampaignTrackingIgnoreList::ACTION_CLICK;
        $ignored->ip_address = $ipAddress;
        $ignored->reason     = t('app', 'The campaign track ignore link was clicked');
        $ignored->save();
    ...
    public function actionReport_abuse($campaign_uid, $list_uid, $subscriber_uid)
    {
        ...
        if (request()->getIsPostRequest() && ($attributes = (array)request()->getPost($report->getModelName(), []))) {
            ...
            // since 2.1.15
            $exists = CampaignAbuseReport::model()->findByAttributes([
                'campaign_id'   => $campaign->campaign_id,
                'subscriber_id' => $subscriber->subscriber_id,
            ]);
            if (!empty($exists)) {
                notify()->addWarning(
                    t('campaigns', 'We found another report from you, we will take proper actions against this as soon as possible!')
                );
            } elseif ($report->save()) {
                /**
                 * @since 2.1.15
                 * @see https://github.com/onetwist-software/mailwizz/issues/756
                 */
                if ($subscriber->getIsConfirmed()) {
                    ...
                    $trackUnsubscribe->note          = EmailBlacklist::ABUSE_COMPLAINT_REASON;
    ...
    {
        if ($action->getId() === 'campaigns/web_version') {
