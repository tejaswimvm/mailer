...
class DswhController extends Controller
{
    ...
    public function actionIndex($id)
    {
        ...
        $map = [
            ...
            'brevo-web-api'        => [$this, 'processBrevo'],
        ...
        $events = collect([])
            ...
            ->merge(
                collect(explode(PHP_EOL, $postedEvents))->filter(function ($line) {
                    ...
                    // since 2.5.0 - translate any variation of the message-id header to our own representation
                    foreach ($event as $key => $value) {
                        if (strtolower((string)$key) !== strtolower($this->pmtaMessageIdHeaderKey)) {
                            continue;
                        }
                        unset($event[$key]);
                        $event[$this->pmtaMessageIdHeaderKey] = $value;
                        break;
                    }
                    return $event;
    ...
    public function processSendgrid()
    {
        ...
        foreach ($events as $evt) {
            ...
            if (in_array($evt['event'], ['bounce'])) {
                ...
                // https://www.twilio.com/docs/sendgrid/for-developers/tracking-events/event#blocked
                if (isset($evt['type']) && $evt['type'] == 'blocked') {
                    // https://www.twilio.com/docs/sendgrid/ui/analytics-and-reporting/bounce-and-block-classifications
                    $classification = $evt['bounce_classification'] ?? '';
                    if (in_array($classification, ['Technical', 'Content', 'Reputation', 'Frequency/Volume Too High'])) {
                        $bounceType = CampaignBounceLog::BOUNCE_INTERNAL;
                    } elseif (in_array($classification, ['Mailbox Unavailable', 'Unclassified'])) {
                        $bounceType = CampaignBounceLog::BOUNCE_SOFT;
                    }
                }
     ...
     * @throws CException
     */
    public function processBrevo()
    {
        $this->processSendinblue();
    }
    ...
    public function processPostmark()
    {
        ...
        // and https://postmarkapp.com/support/article/1065-what-is-the-difference-between-a-spam-complaint-and-a-spam-notification-bounce
        $bounceTypes = [
        ...
        if (in_array($event['Type'], $bounceTypes)) {
            ...
            $mapping = [
                CampaignBounceLog::BOUNCE_INTERNAL => ['Blocked', 'SMTPApiError', 'InboundError', 'DMARCPolicy', 'SpamNotification'],
        ...
        if (in_array($event['Type'], ['SpamComplaint'])) {
            /** @var OptionCronProcessFeedbackLoopServers $fbl */
            $fbl = container()->get(OptionCronProcessFeedbackLoopServers::class);
            $fbl->takeActionAgainstSubscriberWithCampaign($subscriber, $campaign);
            app()->end();
    ...
    protected function beforeAction($action)
    {
        if ($action->getId() !== 'index') {
            ...
            // since 2.3.5
            hooks()->doAction('dswh_before_process', new CAttributeCollection([
                'processor' => $action->getId(),
            ]));

            // since 2.3.5
            app()->getEventHandlers('onEndRequest')->insertAt(0, function () use ($action) {
                hooks()->doAction('dswh_after_process', new CAttributeCollection([
                    'processor' => $action->getId(),
                ]));
            });
