 ...
 */
class ListFieldBuilderTypeTextCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldText', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            /** @var ListFieldText|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                /** @var ListFieldText $model */
                $model = ListFieldText::model()->findByAttributes([
        ...
        if (!$hasEmailTag) {
            $this->owner->errors[] = [
                'show'      => true,
                'message'   => t('list_fields', 'Missing the EMAIL tag, this is not acceptable!'),
                'errors'    => [],
            ];
        } else {
            ...
            if (empty($this->owner->errors)) {
                ...
                ListFieldText::model()->deleteAll($criteria);
            }
        }
    ...
    public function _addReadOnlyAttributes(CEvent $event)
    {
        if ($event->sender->owner->tag === 'EMAIL') {
            if (in_array($event->params['attribute'], ['tag', 'required', 'visibility'])) {
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_text_crud_build_field_array', [
            'field'      => $model,
