...
class ListFieldBuilderTypeTextSubscriber extends ListFieldBuilderTypeSubscriber
{
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        if (empty($this->owner->errors)) {
            $subscriber->email = (string)request()->getPost('EMAIL', (string)$subscriber->email);
            if (!$subscriber->save()) {
                $this->owner->errors[] = [
                    'show'      => true,
                    'message'   => $subscriber->shortErrors->getAllAsString(),
                    'errors'    => $subscriber->shortErrors->getAll(),
                ];
            }
        }
    }
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        if ($event->sender->field->tag === 'EMAIL') {
            ...
            $rules->add(['value', 'email', 'checkMX' => false, 'checkPort' => false]);
    ...
    protected function getValueModels(): array
    {
        ...
        /** @var ListFieldText[] $models */
        $models = ListFieldText::model()->findAllByAttributes([
        ...
        foreach ($models as $model) {
            ...
            $defaultValue = empty($valueModel->value) ? ListFieldText::parseDefaultValueTags((string)$model->default_value, $subscriber) : $valueModel->value;
    ...
    protected function buildFieldArray(ListFieldValue $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_text_subscriber_build_field_array', [
            'field'      => $field,
