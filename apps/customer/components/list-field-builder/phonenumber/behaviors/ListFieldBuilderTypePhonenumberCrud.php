...
/**
 * ListFieldBuilderTypePhonenumberCrud
 *
 ...
 */
class ListFieldBuilderTypePhonenumberCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldPhoneNumber', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            /** @var ListFieldPhoneNumber|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                /** @var ListFieldPhoneNumber $model */
                $model = ListFieldPhoneNumber::model()->findByAttributes([
            ...
            if (empty($model)) {
                $model = new ListFieldPhoneNumber();
            }
        ...
        if (empty($this->owner->errors)) {
            ...
            ListFieldPhoneNumber::model()->deleteAll($criteria);
        }
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_phonenumber_crud_build_field_array', [
            'field'      => $model,
