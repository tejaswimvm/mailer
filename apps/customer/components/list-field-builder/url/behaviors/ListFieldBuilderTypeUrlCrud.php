 ...
 */
class ListFieldBuilderTypeUrlCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldUrl', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            /** @var ListFieldUrl|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                /** @var ListFieldUrl $model */
                $model = ListFieldUrl::model()->findByAttributes([
        ...
        if (empty($this->owner->errors)) {
            ...
            ListFieldUrl::model()->deleteAll($criteria);
        }
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_url_crud_build_field_array', [
            'field'      => $model,
