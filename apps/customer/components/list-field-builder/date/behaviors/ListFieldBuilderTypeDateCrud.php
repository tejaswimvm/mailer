...
/**
 * @property ListFieldBuilderTypeDate $owner
 */
class ListFieldBuilderTypeDateCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    {
        $tags = implode(', ', array_map('html_encode', array_map('strval', array_keys(ListField::getDefaultValueTags()))));
        $text = t('list_fields', 'In case this field is not required and you need a default value for it. The format must be "Y-m-d". Following tags are recognized: {tags}', ['{tags}' => $tags]);
        $event->params['texts']['default_value'] = $text;
    }
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_date_crud_build_field_array', [
            'field'      => $model,
