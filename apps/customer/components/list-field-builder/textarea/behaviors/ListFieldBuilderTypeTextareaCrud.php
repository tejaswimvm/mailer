...
/**
 * ListFieldBuilderTypeTextareaCrud
 *
 ...
 */
class ListFieldBuilderTypeTextareaCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldTextarea', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            /** @var ListFieldTextarea|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                /** @var ListFieldTextarea $model */
                $model = ListFieldTextarea::model()->findByAttributes([
        ...
        if (empty($this->owner->errors)) {
            ...
            ListFieldTextarea::model()->deleteAll($criteria);
        }
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_textarea_crud_build_field_array', [
            'field'      => $model,
