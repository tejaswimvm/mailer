...
/**
 * ListFieldBuilderTypeCheckboxlistCrud
 *
 ...
 */
class ListFieldBuilderTypeCheckboxlistCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldCheckboxlist', []);
        ...
        /** @var ListFieldCheckboxlist[] $models */
        $models = [];

        foreach ($postModels[$typeName] as $index => $attributes) {
            /** @var ListFieldCheckboxlist|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                /** @var ListFieldCheckboxlist $model */
                $model = ListFieldCheckboxlist::model()->findByAttributes([
            ...
            if (empty($model)) {
                $model = new ListFieldCheckboxlist();
            }
                ...
                foreach ($options as $option) {
                    ...
                    if (!$option->save()) {
                        $this->owner->errors[] = [
                            ...
                            'errors'    => $option->shortErrors->getAll(),
                        ];
                    } else {
        ...
        if (empty($this->owner->errors)) {
            ...
            ListFieldCheckboxlist::model()->deleteAll($criteria);
    ...
    public function _displayFields(CEvent $event)
    {
        ...
        /** @var ListFieldCheckboxlist[] $models */
        $models = ListFieldCheckboxlist::model()->findAllByAttributes([
            'type_id' => (int)$fieldType->type_id,
            'list_id' => (int)$list->list_id,
        ]);
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_checkboxlist_crud_build_field_array', [
            'field'      => $model,
