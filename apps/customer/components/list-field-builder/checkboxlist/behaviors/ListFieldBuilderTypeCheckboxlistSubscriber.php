...
class ListFieldBuilderTypeCheckboxlistSubscriber extends ListFieldBuilderTypeSubscriber
{
    ...
    protected function getValueModels(): array
    {
        ...
        foreach ($models as $model) {
            ...
            $defaultValue = !empty($model->default_value) ? array_map(function (string $item) use (
                $subscriber
            ): string {
                return (string)ListFieldCheckboxlist::parseDefaultValueTags($item, $subscriber);
            }, array_map('strval', explode(',', $model->default_value))) : [];
            $defaultValue = array_map('trim', $defaultValue);

            $hasOptionsSet = $subscriber->getListFieldValueModel()->countByAttributes([
    ...
    protected function buildFieldArrayMultiValues(array $models)
    {
        ...
        /** @var ListFieldCheckboxlist $field */
        ...
        if (!$field->getVisibilityIsNone() || apps()->isAppName('customer')) {
            ...
            $fieldHtml = $this->owner->renderInternal(
                $viewFile,
                compact('model', 'field', 'values', 'options', 'visible'),
                true
            );
        }

        return (array)hooks()->applyFilters('list_field_builder_type_checkboxlist_subscriber_build_field_array', [
            'field'      => $field,
