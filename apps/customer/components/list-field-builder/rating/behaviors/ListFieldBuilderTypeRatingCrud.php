...
/**
 * ListFieldBuilderTypeRatingCrud
 *
 ...
 */
class ListFieldBuilderTypeRatingCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldRating', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = ListFieldRating::model()->findByAttributes([
        ...
        if (empty($this->owner->errors)) {
            ...
            ListFieldRating::model()->deleteAll($criteria);
        }
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_rating_crud_build_field_array', [
            'field'      => $model,
