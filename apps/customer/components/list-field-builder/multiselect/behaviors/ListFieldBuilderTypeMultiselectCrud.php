...
/**
 * ListFieldBuilderTypeMultiselectCrud
 *
 ...
 */
class ListFieldBuilderTypeMultiselectCrud extends ListFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('ListFieldMultiselect', []);
        ...
        /** @var ListFieldMultiselect[] $models */
        $models = [];

        foreach ($postModels[$typeName] as $index => $attributes) {
            /** @var ListFieldMultiselect|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                /** @var ListFieldMultiselect $model */
                $model = ListFieldMultiselect::model()->findByAttributes([
            ...
            if (empty($model)) {
                $model = new ListFieldMultiselect();
            }
        ...
        if (empty($this->owner->errors)) {
            ...
            ListFieldMultiselect::model()->deleteAll($criteria);
    ...
    public function _displayFields(CEvent $event)
    {
        ...
        /** @var ListFieldMultiselect[] $models */
        $models = ListFieldMultiselect::model()->findAllByAttributes([
            'type_id' => (int)$fieldType->type_id,
            'list_id' => (int)$list->list_id,
        ]);
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_multiselect_crud_build_field_array', [
            'field'      => $model,
