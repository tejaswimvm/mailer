...
class ListFieldBuilderTypeConsentCheckboxCrud extends ListFieldBuilderTypeCrud
{
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        /** @var array $originalPostModels */
        $originalPostModels = request()->getOriginalPost('ListFieldConsentCheckbox', []);
        ...
        foreach ($postModels[$typeName] as $index => $attributes) {
            ...
            // since 2.2.9
            $unfilteredAttributes = $originalPostModels[$typeName][$index] ?? [];
            if (isset($unfilteredAttributes['consent_text'])) {
                $attributes['consent_text'] = ioFilter()->purify($unfilteredAttributes['consent_text']);
            }
    ...
    {
        if (!in_array($event->params['attribute'], ['consent_text'])) {
            return;
        }
        ...
        /** @var ListFieldConsentCheckbox $owner */
        $owner = $event->sender->owner;

        if ($owner->getIsNewRecord()) {
            $activeId = CHtml::activeId($owner, $event->params['attribute']);
        } else {
            $activeId = CHtml::activeId($owner, sprintf('%s_%d_%s', $owner->type->identifier, $owner->currentIndex, $event->params['attribute']));
        }
    ...
    protected function buildFieldArray(ListField $model): array
    {
        ...
        $consentModel->currentIndex = $index;

        $viewFile = realpath(__DIR__ . '/../views/field-tpl.php');
        $consentModel->fieldDecorator->onHtmlOptionsSetup = [$this, '_setupEditorOptions'];
        ...
        return (array)hooks()->applyFilters('list_field_builder_type_consent_checkbox_crud_build_field_array', [
            'field'      => $consentModel,
