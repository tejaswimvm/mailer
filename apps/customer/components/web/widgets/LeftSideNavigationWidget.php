...
class LeftSideNavigationWidget extends CWidget
{
    ...
    public function getMenuItems()
    {
        ...
        $menuItems = [
            ...
            'landing_pages' => [
                'name'      => t('app', 'Landing pages'),
                'icon'      => 'fa-clipboard',
                'active'    => ['landing_pages', 'landing_page_domains'],
                'route'     => null,
                'items'     => [
                    ['url' => ['landing_pages/index'], 'label' => t('app', 'Landing pages'), 'active' => in_array($route, ['landing_pages/index', 'landing_pages/create', 'landing_pages/update'])],
                    ['url' => ['landing_page_domains/index'], 'label' => t('app', 'Domains'), 'active' => in_array($route, ['landing_page_domains/index', 'landing_page_domains/create', 'landing_page_domains/update'])],
                ],
            ],
            'servers'       => [
                'name'      => t('app', 'Servers'),
            ...
            'surveys' => [
                ...
                'route'     => null,
                'items'     => [
                    ['url' => ['surveys_categories/index'], 'label' => t('app', 'Categories'), 'active' => strpos($route, 'surveys_categories') === 0],
                    ['url' => ['surveys/index'], 'label' => t('app', 'Surveys'), 'active' => in_array($route, ['surveys/index', 'surveys/create', 'surveys/update'])],
                ],
            ],
            'articles' => [
                'name'      => t('app', 'Articles'),
        ...
        if (!$maxDeliveryServers && !$maxBounceServers && !$maxFblServers && !$maxEmailBoxMonitors) {
            ...
            if (!$maxDeliveryServers && isset($menuItems['servers']['items'][$index + 1])) {
        ...
        // landing pages
        if ($customer->getGroupOption('landing_pages.max_landing_pages', -1) == 0) {
            $menuItems['landing_pages']['visible'] = false;
        }
        ...
        if ($customer->getGroupOption('campaigns.can_use_autoresponders', 'yes') != 'yes') {
            $menuItems['campaigns']['items'][2]['visible'] = false;
        }
        ...
        // since 2.2.19 - templates gallery
        if ((int)CustomerEmailTemplate::model()->count('customer_id IS NULL') === 0) {
            $menuItems['templates']['items'][2]['visible'] = false;
        }

        // filter out the items which should not be visible
        $menuItems = collect($menuItems)->map(function (array $item): array { // @phpstan-ignore-line
