jQuery(document).ready(function($){
	...
	// delete field button.
	$(document).on('click', 'a.btn-remove-rating-field', function(){
	...
	// add
	$('a.btn-add-rating-field').on('click', function(){
		var currentIndex = -1;
		...
		currentIndex++;
		var tpl = $('#field-rating-javascript-template').html();
		tpl = tpl.replace(/\{index\}/g, currentIndex);
		$tpl = $(tpl);
		$('.survey-fields').append($tpl);
		
		$tpl.find('.has-help-rating').popover();
