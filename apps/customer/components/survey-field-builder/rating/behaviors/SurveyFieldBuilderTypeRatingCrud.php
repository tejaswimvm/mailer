...
/**
 * SurveyFieldBuilderTypeRatingCrud
 *
 ...
 */
class SurveyFieldBuilderTypeRatingCrud extends SurveyFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array) request()->getPost('SurveyFieldRating', []);
        if (!isset($postModels[$typeName]) || !is_array($postModels[$typeName])) {
            $postModels[$typeName] = [];
        }

        /** @var SurveyFieldRating[] $models */
        $models = [];

        foreach ($postModels[$typeName] as $attributes) {
            /** @var SurveyFieldRating|null $model */
            $model = null;

            if (!empty($attributes['field_id'])) {
                $model = SurveyFieldRating::model()->findByAttributes([
        ...
        if (empty($this->owner->errors)) {
            ...
            SurveyFieldRating::model()->deleteAll($criteria);
        }
