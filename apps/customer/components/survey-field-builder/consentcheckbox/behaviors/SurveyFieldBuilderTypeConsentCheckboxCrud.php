...
/**
 * SurveyFieldBuilderTypeConsentCheckboxCrud
 *
 ...
 */
class SurveyFieldBuilderTypeConsentCheckboxCrud extends SurveyFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array) request()->getPost('SurveyFieldConsentCheckbox', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = SurveyFieldConsentCheckbox::model()->findByAttributes([
