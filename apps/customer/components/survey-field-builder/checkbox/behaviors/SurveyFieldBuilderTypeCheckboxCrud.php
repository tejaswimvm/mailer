...
/**
 * SurveyFieldBuilderTypeCheckboxCrud
 *
 ...
 */
class SurveyFieldBuilderTypeCheckboxCrud extends SurveyFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array)request()->getPost('SurveyFieldCheckbox', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = SurveyFieldCheckbox::model()->findByAttributes([
        ...
        if (empty($this->owner->errors)) {
            ...
            SurveyFieldCheckbox::model()->deleteAll($criteria);
        }
