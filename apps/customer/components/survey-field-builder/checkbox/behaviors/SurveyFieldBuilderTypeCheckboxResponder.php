...
class SurveyFieldBuilderTypeCheckboxResponder extends SurveyFieldBuilderTypeResponder
{
    ...
    protected function getValueModels(): array
    {
        ...
        foreach ($models as $model) {
            ...
            // since 2.2.17
            $model->setDefaultValueParsed((string)SurveyFieldCheckbox::parseDefaultValueTags((string)$model->default_value, $responder));

            /** set the correct default value. */
            $defaultValue = strlen(trim((string)$valueModel->value)) == 0 ? (string)$model->getDefaultValueParsed() : (string)$valueModel->value;

            /** assign props */
            $valueModel->field         = $model;
            $valueModel->field_id      = (int)$model->field_id;
            $valueModel->responder_id  = (int)$responder->responder_id;
            $valueModel->value         = (string)request()->getPost($model->getTag(), $defaultValue);
    ...
    protected function buildFieldArray(SurveyFieldValue $model): array
    {
        ...
        return (array)hooks()->applyFilters('survey_field_builder_type_checkbox_responder_build_field_array', [
