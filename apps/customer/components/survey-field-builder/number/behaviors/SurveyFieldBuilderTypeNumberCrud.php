...
/**
 * SurveyFieldBuilderTypeNumberCrud
 *
 ...
 */
class SurveyFieldBuilderTypeNumberCrud extends SurveyFieldBuilderTypeCrud
{
    /**
     * @param CEvent $event
     *
    ...
    public function _saveFields(CEvent $event)
    {
        ...
        $postModels = (array) request()->getPost('SurveyFieldNumber', []);
        ...
        foreach ($postModels[$typeName] as $attributes) {
            ...
            if (!empty($attributes['field_id'])) {
                $model = SurveyFieldNumber::model()->findByAttributes([
        ...
        if (empty($this->owner->errors)) {
            ...
            SurveyFieldNumber::model()->deleteAll($criteria);
        }
