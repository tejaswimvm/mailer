...
class CampaignStatsProcessorBehavior extends CBehavior
{
     ...
     */
    public const CACHE_SHORT = 300; // 5 mins

    /**
     * Amount in seconds for medium caches
     */
    public const CACHE_MEDIUM = 21600; // 6 hours

    /**
     * Amount in seconds for long caches
     */
    public const CACHE_LONG = 604800; // 7 days

    /**
     * Cache forever
     */
    public const CACHE_FOREVER = 0;
    ...
    {
        $cacheKey = sha1(self::class . '::_getOpensCount' . get_class($this->owner) . $this->owner->campaign_id . 'opens');
    ...
    {
        $cacheKey = sha1(self::class . '::_getUniqueOpensCount' . get_class($this->owner) . $this->owner->campaign_id . 'opens-unique');
        return $this->deleteFromCache($cacheKey);
    }
    ...
    {
        $cacheKey = sha1(self::class . '::_getClicksCount' . get_class($this->owner) . $this->owner->campaign_id);
        return $this->deleteFromCache($cacheKey);
    }
    ...
    {
        $cacheKey = sha1(self::class . '::_getUniqueClicksCount' . get_class($this->owner) . $this->owner->campaign_id);
        return $this->deleteFromCache($cacheKey);
    }
    ...
    public function _getIndustryProcessedCount(): int
    {
        ...
        $cdlModel = $this->owner->getDeliveryLogsArchived() ? CampaignDeliveryLogArchive::model() : $owner->getCampaignDeliveryLogModel();
    ...
    protected function _getDeliverySuccessRate()
    {
        ...
        if ($this->_deliverySuccessRate > 100) {
            $this->_deliverySuccessRate = 100.0;
        }
    ...
    protected function _getDeliveryErrorRate()
    {
        ...
        if ($this->_deliveryErrorRate > 100) {
            $this->_deliveryErrorRate = 100.0;
        }
    ...
    protected function _getOpensRate()
    {
        ...
        if ($this->_opensRate > 100) {
            $this->_opensRate = 100.0;
        }
    ...
    protected function _getUniqueOpensRate()
    {
        ...
        if ($this->_uniqueOpensRate > 100) {
            $this->_uniqueOpensRate = 100.0;
        }
    ...
    protected function _getBouncesRate()
    {
        ...
        if ($this->_bouncesRate > 100) {
            $this->_bouncesRate = 100.0;
        }
    ...
    protected function _getHardBouncesRate()
    {
        ...
        if ($this->_hardBouncesRate > 100) {
            $this->_hardBouncesRate = 100.0;
        }
    ...
    protected function _getSoftBouncesRate()
    {
        ...
        if ($this->_softBouncesRate > 100) {
            $this->_softBouncesRate = 100.0;
        }
    ...
    protected function _getInternalBouncesRate()
    {
        ...
        if ($this->_internalBouncesRate > 100) {
            $this->_internalBouncesRate = 100.0;
        }
    ...
    protected function _getUnsubscribesRate()
    {
        ...
        if ($this->_unsubscribesRate > 100) {
            $this->_unsubscribesRate = 100.0;
        }
    ...
    protected function _getComplaintsRate()
    {
        ...
        if ($this->_complaintsRate > 100) {
            $this->_complaintsRate = 100.0;
        }
    ...
    protected function _getIndustryOpensRate()
    {
        ...
        if ($this->_industryOpensRate > 100) {
            $this->_industryOpensRate = 100.0;
        }
