...
class List_segmentsController extends Controller
{
    ...
    public function actionIndex($list_uid)
    {
        ...
        if ($segment->getIsArchived()) {
            $pageHeading = t('list_segments', 'Archived segments');
            $breadcrumbs[t('list_segments', 'Archived segments')] = createUrl('list_segments/index', ['list_uid' => $list->list_uid, 'ListSegment[status]' => ListSegment::STATUS_ARCHIVED]);
        }
    ...
    public function actionSubscribers($list_uid, $segment_uid)
    {
        ...
        foreach ($subscribers as $subscriber) {
            ...
            foreach ($fields as $field) {
                ...
                $values = $subscriber->getListFieldValueModel()->findAll($criteria);
                ...
                $subscriberRow['columns'][] = ioFilter()->purify(implode($field->getMultiValuesSeparator(), $value));
            }
     ...
     * @throws CException
     */
    public function actionToggle_archive($list_uid, $segment_uid)
    {
        /** @var Lists $list */
        $list = $this->loadListModel((string)$list_uid);
        ...
        $returnRoute = ['list_segments/index', 'list_uid' => $list->list_uid];

        if ($segment->getIsPendingDelete()) {
            $this->redirect($returnRoute);
        }

        if ($segment->getIsArchived()) {
            $segment->saveStatus(ListSegment::STATUS_ACTIVE);
            notify()->addSuccess(t('list_segments', 'Your list segment was successfully unarchived!'));
            $returnRoute = ['list_segments/index', 'list_uid' => $list_uid];
        } else {
            $segment->saveStatus(ListSegment::STATUS_ARCHIVED);
            notify()->addSuccess(t('list_segments', 'Your list segment was successfully archived!'));
            $returnRoute = ['list_segments/index', 'list_uid' => $list->list_uid, 'ListSegment[status]' => ListSegment::STATUS_ARCHIVED];
        }
