...
class List_fieldsController extends Controller
{
    ...
    public function init()
    {
        ...
        // since 2.3.9
        $this->addPageScript(['src' => AssetsUrl::js('list-fields.js')]);
    ...
    public function actionIndex($list_uid)
    {
        ...
        if (request()->getIsPostRequest()) {
            ...
            try {
                // since 2.3.9
                if ($maxListFields > -1) {
                    $requestFieldsCount = (int)collect(request()->getPost('', []))
                        ->filter(function ($data, $fieldClass): bool {
                            return stripos($fieldClass, ListField::class) === 0;
                        })
                        ->reduce(function ($carry, array $data): int {
                            return $carry + collect($data)->map(function ($fields): int {
                                return count($fields);
                            })->sum();
                        }, 0);
                    if ($requestFieldsCount > $maxListFields) {
                        throw new Exception(t('list_fields', 'You have reached the maximum number of allowed custom fields. Your list has {current} custom fields and you are allowed only {max} custom fields.', [
                            '{current}' => $requestFieldsCount,
                            '{max}'     => $maxListFields,
                        ]));
                    }
                }
        ...
        $this->render('index', compact('fieldsHtml', 'list', 'maxListFields', 'listFieldsCount'));
    }
