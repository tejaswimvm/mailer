...
class ListsController extends Controller
{
    ...
    public function actionAll_subscribers()
    {
        ...
        if ($filter->getIsExportAction() && $canExport) {
            // since 2.1.18
            $filterAttributes = $filter->getAttributes(array_merge($filter->getSafeAttributeNames(), ['customer_id']));
            if (is_subaccount() && subaccount()->customer()) {
                $filterAttributes['subaccount_customer_id'] = (int)subaccount()->customer()->customer_id;
            }

            queue_send('customer.lists.allsubscribers.filter.export', $filterAttributes);
