...
jQuery(document).ready(function($){
    ...
    if ($sendAt.length && $displaySendAt.length && $fakeSendAt.length) {
        ...
        function syncDateTime() {
            ...
            $.get($fakeSendAt.data('syncurl'), {date: date}, function(json){
                ...
                if ($fakeSendAt.data('cannotifyreset')) {
                    notify.remove();
                }
                $fakeSendAt.data('cannotifyreset', true);
                if (json.beforeCurrentTime && json.beforeCurrentTime.is && json.beforeCurrentTime.message) {
                    notify.addWarning(json.beforeCurrentTime.message).show();
                }
            }, 'json');
        }
        syncDateTime();
    ...
    const btnTemplateClickActionsListFieldsAdd = function(_opts){
        ...
        if (options.canAddAll) {
            const labelAll = $tpl.find('.is788-url').data('label-all');
            if (labelAll) {
                const all = new Option(labelAll, 'all');
                $tpl.find('.is788-url option:first').after(all);
                $tpl.find('.is788-url').on('change', function(){
                    if ($(this).val() !== 'all') {
                        return;
                    }
                    const $wrapper = $(this).closest('.template-click-actions-list-fields-row');
                    const fieldIdValue = $wrapper.find('.is788-field-id').val();
                    const fieldValue = $wrapper.find('.is788-field-value').val();
                    if (fieldIdValue === "" || fieldValue === "") {
                        $wrapper.find('.is788-url').val('');
                        alert($tpl.find('.is788-url').data('message-missing-all-values'));
                        return false;
                    }

                    let insertedRows = 0;
                    $wrapper.find('.is788-url').find('option').each(function() {
                        const urlValue = $(this).attr('value');
                        if (urlValue === '' || urlValue === 'all') {
                            return;
                        }
                        let duplicate = false;
                        $('.template-click-actions-list-fields-list .template-click-actions-list-fields-row').each(function() {
                            if (duplicate) {
                                return;
                            }
                            const _urlValue = $(this).find('.is788-url').val();
                            const _fieldIdValue = $(this).find('.is788-field-id').val();
                            const _fieldValue = $(this).find('.is788-field-value').val();
                            if (_urlValue === urlValue && _fieldIdValue === fieldIdValue && _fieldValue === fieldValue) {
                                duplicate = true;
                            }
                        });
                        if (duplicate) {
                        ...
                        btnTemplateClickActionsListFieldsAdd({
                            urlValue: urlValue,
        ...
        if (options.urlValue) {
            $tpl.find('.is788-url').val(options.urlValue);
        }
        if (options.fieldIdValue) {
            $tpl.find('.is788-field-id').val(options.fieldIdValue);
        }
        if (options.fieldValue) {
            $tpl.find('.is788-field-value').val(options.fieldValue);
        }
        //
    ...
    $('.template-click-actions-list-fields-container a.btn-template-click-actions-list-fields-remove-all').on('click', function() {
        const $els = $('.template-click-actions-list-fields-container .template-click-actions-list-fields-row');
        if (!$els.length || !confirm($(this).data('message'))) {
            return false;
        }
        $els.remove();
        $('button.btn-template-click-actions-list-fields span.count').text('0');
        return false;
    });
    //
    ...
    $(document).on('click', '#btn-run-bulk-action', function(e) {
        ...
        if ($('#bulk_action').val() === 'share-campaign-code') {
            $('#bulk-campaigns-share-code').modal('show');
            return false;
        }
    ...
    $(document).on('submit', '#bulk-campaigns-share-code-form', function(){

        $('#bulk-action-form')
            .append($('<input/>').attr({name: 'bulk_action'}).val($('#bulk_action').val()))
            .append($('.checkbox-column input[type=checkbox]:checked').clone())
            .append($('<input/>').attr({name: 'CampaignShareCode[allowed_usage]'}).val($('#CampaignShareCode_allowed_usage').val()))
            .submit();
    ...
    (function() {
        const $els = [
            ...
            $('#campaign-overview-delivery-stats-wrapper'),
        ];
        $els.map(function($el) {
            if (!$el.length) {
                return;
    ...
    // since 2.1.20
    $(document).on('click', '.btn-send-between-reset-selection', function() {
        $(this).closest('.row').find('.form-group select').val('').trigger('change');
        return false;
    });
    ...
    (function(){
        const handler = () => {
            if ($('#CampaignOption_smart_open_tracking').val() === 'yes' || $('#CampaignOption_smart_click_tracking').val() === 'yes') {
                $('.campaign-smart-tracking-info-wrapper').show();
            } else {
                $('.campaign-smart-tracking-info-wrapper').hide();
            }
        };
        $('#CampaignOption_smart_open_tracking, #CampaignOption_smart_click_tracking').on('change', handler);
        handler();
    })();
