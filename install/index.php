...
// always have index.php and the route in the URL to avoid any possible issue with web servers configuration
if (empty($route) || (!empty($_SERVER['REQUEST_URI']) && stripos($_SERVER['REQUEST_URI'], 'index.php') === false)) {
	header('Location: ' . AppInitHelper::getBaseUrl('index.php?route=welcome'));
	exit;
}
